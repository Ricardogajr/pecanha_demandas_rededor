...
User Function F0702601(oCabec, oCorpo, nOperac)
	...
	// BLOCO NOVO
	// @Author  Sato
	// @Since   02/01/2026 14:52:29
	// @Review  27/02/2026
	// ------------------------------------
	// Adicionado para atender a demanda de melhoria do projeto WPD - Envio de email automatico para fornecedor.
	// ------------------------------------
	Local lPcWPD    := .F.
	Local cRecnoSC7 := 0
	Local nPosEmail := 0
	...
	// @Review  27/02/2026
	// ------------------------------------
	// Adicionado para atender a demanda de melhoria do projeto TASY/OPMENEXO - Envio de email automatico para fornecedor.
	// ------------------------------------
	Local cOpnexo := ""
	Local lOpnexo := .F.
	// ------------------------------------
...
//		End Transaction
		...
		*/
		...
		If nOperac == 3
			...
			// BLOCO NOVO
			// @Author  Sato
			// @Since   06/11/2025 13:15:45
			// @Review  15/12/2025
			// ------------------------------------
			// Adicionado para atender a demanda de melhoria do projeto Netsuite:
			// - Foi criado o parametro MV_IDORINT, onde é informado o código do comprador para que 
			//   no momento da inclusão do Pedido de Compra, seja criada a alçada de aprovação
			//   de acordo com o Grupo de Compras do comprador informado.
			//
			// Para os demais processos, seguirá utilizando o comprador 001 - TOTVS-INTEGRADOR, com 
			// suas respectivas regras de alçada.
			// ------------------------------------
			If SubStr(Upper(AllTrim(oCabec:cXFRONT)), 1, 3) == "NTS"
				cUserCpBio := SuperGetMv("MV_XIDORCP",,"")
				cCodCpr := Posicione("SY1", 03, xFilial("SC1") + cUserCpBio, "Y1_COD")
			Else
				cCodCpr := SuperGetMv("FS_CPRINTE",,"")
			EndIf
			...
			If Empty(cCodCpr)
				If SubStr(Upper(AllTrim(oCabec:cXFRONT)), 1, 3) == "NTS"
					cRet := "Parametro MV_XIDORCP não informado para a Filial " + cFilAnt + "!"
				Else
					cRet := "Parametro FS_CPRINTE não informado para a Filial " + cFilAnt + "!"
				EndIf
				lRet := .F.
			...
			For nX := 1 To Len(oCorpo)
				...
				// @Review  10/02/2026
				// ------------------------------------
				// Adicionado para atender a demanda de melhoria do projeto TASY - Envio de email automatico para fornecedor.
				// ------------------------------------
				AAdd(aLinha, {"C7_XOCTASY"   , oCabec:cTIPOOC 							            		 , Nil})
				// ------------------------------------
			...
			Begin Transaction
				...
				If SC7->(MsSeek(xFilial("SC7") + cXNum))
					If AllTrim(SC7->C7_XNUM) == cXNum
						...
						Else
							If cEnvMail == 'S'
								nPosEmail := AScan(aItens[1], {|x| AllTrim(x[1]) == "C7_XENVMAL"})
								For nX := 1 To Len(aItens)
									aItens[nX][nPosEmail][2] := cEnvMail
								Next nX
								//AAdd(aLinha, {"C7_XENVMAL", cEnvMail, Nil})
							EndIf
						EndIf
					EndIf
				EndIf
			...
			End Transaction
				...
				SC7->(DbSetOrder(1))
				For nH := 01 to Len(oCorpo)

					cUpd := "UPDATE " + RetSqlName("SC7") + " SET C7_XINFPAC = '"+SubStr(AllTrim(oCorpo[nH]:cINFPAC),1,254)+"' "
					cUpd += "WHERE D_E_L_E_T_ = ' ' AND C7_FILIAL = '"+oCabec:cFilReg+"' AND C7_XNUM = '"+oCabec:cXNUM+"' AND C7_ITEM = '"+AllTrim(Strzero(val(oCorpo[nH]:cITEM), 4))+"'"

					TcSqlExec(cUpd)
					/*/SC7->(DbSeek(xFilial("SC7") + cNum + AllTrim(Strzero(val(oCorpo[nH]:cITEM), 4))))
					SC7->(Reclock("SC7",.F.))
						SC7->C7_XINFPAC :=  
					SC7->(MsUnLock())/*/
				Next nH
				...
				If nOperac == 3
					...
					SC7->( dbGoTop() )
					cRecnoSC7 := 0
					If SC7->( dbseek( oCabec:cFilReg + cNum ) )
						If SubStr(SC7->C7_XFRONT, 1, 3) == "AGF" .and. SC7->C7_XORIG == '2'
							Do While SC7->(!Eof()) .and. SC7->C7_FILIAL == oCabec:cFilReg .and. SC7->C7_NUM == cNum
								If SC7->C7_DATPRF == CtoD("31/12/2100")
									If cRecnoSC7 == 0
										cRecnoSC7 := SC7->( RecNo() )
									EndIf
					...
					// Adicionado para atender a demanda de melhoria do projeto TASY/OPMENEXO - Envio de email automatico para fornecedor.
					// ------------------------------------
					cOpnexo := GetAdvFVal("P33", "P33_XOPNEX", xFilial('P33') + oCabec:cFilReg, 1, "")
					If cOpnexo == "1"
						dbSelectArea("SC7")
						SC7->( dbSetOrder(1) )     // C7_FILIAL + C7_NUM
						SC7->( dbGoTop() )
						cRecnoSC7 := 0
						If SC7->( dbseek( oCabec:cFilReg + cNum ) )
							If SubStr(SC7->C7_XFRONT, 1, 3) == "TSY" .and. SC7->C7_XORIG == '2' .and. SC7->C7_XOCTASY == 'G' 
								Do While SC7->(!Eof()) .and. SC7->C7_FILIAL == oCabec:cFilReg .and. SC7->C7_NUM == cNum 
									If cRecnoSC7 == 0
										cRecnoSC7 := SC7->( RecNo() )
									EndIf
