				...
				Begin Transaction
					...
					If nOperac == 3
						...
						For nZ := 1 to Len(oParcela)
							...
							aTitulo := {}
							lRet := CarArrayCP(oParcela[nZ], cXID, @cRet, @aTitulo, SD1->D1_CC, _cCodServ, nLenParc,nZ)
		...
		Else
			...
			Else
				...
				If SD1->(DbSeek(xFilial("SD1") + Padr(oCabec:cDoc,nTamDoc) + Padr(oCabec:cSerie,nTamSer) + Padr(oCabec:cFornece,nTamFor) + Padr(oCabec:cLoja,nTamLoja)))
					While !(SD1->(EOF())) .And. SD1->(D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA) == (xFilial("SF1") + Padr(oCabec:cDoc,nTamDoc) + Padr(oCabec:cSerie,nTamSer) + Padr(oCabec:cFornece,nTamFor) + Padr(oCabec:cLoja,nTamLoja))
						Reclock("SD1",.F.)
						SD1->(DbDelete())
						SD1->(MSUNLOCK())
						SD1->(DbSkip())
...
Static Function CarArrayNf(oCabec,oCorpo,aCabec,aItens,cRet,cXID,oParcela)
	...
	Else
		AADD(aCabec,{"F1_XDTORIG"	,aVctos[2][1] })
	EndIf
...
*/		
		...
		DbSelectArea("SC7")
		SC7->(DBSETORDER(1))
		SC7->(DBSEEK(cFilAnt+oCorpo[nX]:cPedido+oCorpo[nX]:cItemPC))
		...
		If lFixa
			...
			Else
				AAdd(aLinha,{"D1_XPRIVEN" , aVctos[2][1]			,Nil})
			EndIf
		EndIf
...
Static Function CarArrayCp(oParcela, cXID, cRet, aTitulo, cCCusto, _cCodServ, nLen,nZ)
...
*/
	...
	If lDtCorrige
		AAdd(aTitulo,{"E2_XDTORIG",aVctos[nZ+1][01] 			, Nil})//Melhoria data fixa - Lucas Miranda de Aguiar
	Else
...
Static Function EnviaEmail()
	...
	nRet := oSrv:Init("", cSMTP, cConta, cPass, 0, 25)
	If nRet != 0
		lConSMTP := .F.
	Else
		lConSMTP := .T.
	...
	If lConSMTP
		...
		If !lEnvEmail // Erro no envio do email
			...
			cError := oSrv:GetErrorString(nRet)
