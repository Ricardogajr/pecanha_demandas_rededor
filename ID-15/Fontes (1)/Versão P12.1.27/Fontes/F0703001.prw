...
User Function F07030Executa(cEmp, cFil, aCodFec, lJob)
	...
	If LockByName("F0703001" + cEmp + cFil, .F., .F.)
		If lJob
			...
			PtInternal(1,"Job F07030EX  "+ cEmp+ " " + cFil)
		Else
			aCodFec := BuscaP26Prt()
			cEmpBkp := cEmpAnt
			cFilBkp := cFilAnt
		...
		If lJob
			RpcClearEnv()
		Else
			Aviso("Processo Finalizado","Processo finalizado. Verifique o Log de processamento.",{"Ok"})
			cEmpAnt := cEmpBkp
			cFilAnt := cFilBkp
		EndIf
...
Static Function AtuSB2(cCodFec, dDtFec, cMsg)
	...
	While (cAliasNew)->(!Eof())
		...
		If SB2->(DbSeek( (cAliasNew)->P27_FILIAL + (cAliasNew)->P27_PRODUT + (cAliasNew)->P27_LOCAL ))
			...
			SB2->B2_XQTDE  := CalcEst((cAliasNew)->P27_PRODUT, (cAliasNew)->P27_LOCAL, dDtFec + 1)[ 1 ]
