...
Static Function fExec(aExec)
	...
	Begin Sequence
		...
		While !SX3->(Eof()) .And. AllTrim(SX3->X3_ARQUIVO) == AllTrim(cTabela)
			nPos := aScan(aExec, {|x| AllTrim(x[1]) == AllTrim(SX3->X3_CAMPO)})
			If nPos == 0 .And. AllTrim(SX3->X3_CONTEXT) != "V"
				If X3Obrigat(SX3->X3_CAMPO)
					aAdd(aLog, {"O Campo " + SX3->X3_CAMPO + " é obrigatorio e não esta na arquivo, por favor, verificar.", _nLinha})
					Break
				EndIf
				SX3->(DbSkip())
				Loop
			...
			If AllTrim(SX3->X3_CONTEXT) != "V"
				...
				Else
					If cTabela == "SAI"
						If AllTrim(aExec[2][2]) != "******"
							cCodUsrAi := UsrFullName(AllTrim(aExec[2][2]))
					...
					If !lInc
						...
						Else
							If aExec[nPos][01] $ SuperGetMv("MV_XCPDIF",,"AK_APROSUP|AL_APROSUP")
								(cTabela)->&(aExec[nPos][01]) := aExec[nPos][02]
								M->&(aExec[nPos][01]) := aExec[nPos][02]
							Else
				...
				If X3Obrigat(SX3->X3_CAMPO)
					If Empty(aExec[nPos][02] )
						aAdd(aLog, {"O Campo " + SX3->X3_CAMPO + " é obrigatorio e esta vazio, por favor, verificar o arquivo.", _nLinha})
						If cTabela == "SY1"
							ConfirmSX8()
						EndIf
						Break
				...
				If (!Empty(SX3->X3_VALID) .Or. !Empty(SX3->X3_VLDUSER))
					...
					__ReadVar := cCampoAtu
					cVldPad := SX3->X3_VALID
					cVldUsr := SX3->X3_VLDUSER
					If !Empty(cVldPad)
						If "FwFldGet(" $ cVldPad
							SX3->(DbSkip())
							Loop
