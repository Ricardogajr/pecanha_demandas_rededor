...
while (CALIASTMP)->(.not. (EOF())); 
    ...
    AADD(AAUX,{APA5INF[5],APA5INF[4],ARESPONS[1],ARESPONS[2],ARESPONS[3],ARESPONS[4],ARESPONS[5],ARESPONS[6],ARESPONS[7],POSICIONE("RD0",12,XFILIAL("RD0")+CALIASTMP->RH3_MATINI,"RD0_LOGIN"),CALIASTMP->RH3_CODIGO,CALIASTMP->RH3_MAT,iif( val(CALIASTMP->RH3_STATUS)==1,"Em processo de aprovação",iif( val(CALIASTMP->RH3_STATUS)==2,"Atendida",iif( val(CALIASTMP->RH3_STATUS)==3,"Reprovada","Aguardando Efetivação do RH"))),APA5INF[9],CALIASTMP->RH3_DTATEN,CALIASTMP->RH3_FILINI,CALIASTMP->RH3_FILAPR,CALIASTMP->RH3_XDTAPV,APA5INF[1],APA5INF[2],APA5INF[3],POSICIONE("SRA",1,CALIASTMP->RH3_FILIAL+CALIASTMP->RH3_MAT,"RA_NOME"),POSICIONE("SRA",1,CALIASTMP->RH3_FILIAL+CALIASTMP->RH3_MAT,"RA_CIC"),POSICIONE("SRA",1,CALIASTMP->RH3_FILIAL+CALIASTMP->RH3_MAT,"RA_CARGO"),POSICIONE("SRA",1,CALIASTMP->RH3_FILIAL+CALIASTMP->RH3_MAT,"RA_DEPTO"),POSICIONE("SRA",1,CALIASTMP->RH3_FILIAL+CALIASTMP->RH3_MAT,"RA_CC"),CALIASTMP->RH3_MATINI,POSICIONE("SRA",1,CALIASTMP->RH3_FILINI+CALIASTMP->RH3_MATINI,"RA_NOME"),CCPFSOLIC,POSICIONE("PAB",1,XFILIAL("PAB")+CALIASTMP->RH3_XCODAL,"PAB_SOLDES"),POSICIONE("PAB",1,XFILIAL("PAB")+CALIASTMP->RH3_XCODAL,"PAB_GRPDES"),APA5INF[6],ACANDIDATO[1],ACANDIDATO[2],APA5INF[7],APA5INF[8]})
...
while .not. ((CALIPA5)->(EOF())); 
    ...
    ; if .not. (EMPTY(CALIPA5->PA5_XUNAPR)) .and. ( alltrim(NOACENTO(CALIPA5->PA5_XSTAT))) $ (NOACENTO("EFETIVAÇÃO ATENDIDA"))
        CXUNIDA := CALIPA5->PA5_XUNAPR
    else 
        CXUNIDA := ""
    endif
    ; if .not. (EMPTY(CALIPA5->PA5_XAPROV)) .and. ( alltrim(NOACENTO(CALIPA5->PA5_XSTAT))) $ (NOACENTO("EFETIVAÇÃO ATENDIDA"))
        CXAPROV := CALIPA5->PA5_XAPROV
    else 
        CXAPROV := ""
    endif
    ; if .not. (EMPTY(CALIPA5->PA5_XSEAPV)) .and. ( alltrim(NOACENTO(CALIPA5->PA5_XSTAT))) $ (NOACENTO("EFETIVAÇÃO ATENDIDA"))
        CXSEAPV := CALIPA5->PA5_XSEAPV
    else 
        CXSEAPV := ""
    endif
    ; if .not. (EMPTY(CALIPA5->PA5_XSTAT)) .and. ( alltrim(NOACENTO(CALIPA5->PA5_XSTAT))) $ (NOACENTO("EFETIVAÇÃO ATENDIDA"))
