...
Static Function MenuDef()
	...
	Local aRotina := {}
			...
			AAdd(aRotina,{"Pesquisar" , 'VIEWDEF.F0100401'   , 0, 1 } )
			AAdd(aRotina,{"Visualizar" , 'U_xFILSF(1)'   , 0, 2 } )
			AAdd(aRotina,{"Incluir" , 'U_xFILSF(3)'   , 0, 3 } )
		    AAdd(aRotina,{"Alterar" , 'U_xFILSF(4)'   , 0, 4 } )
			AAdd(aRotina,{"Excluir" , 'U_xFILSF(5)'   , 0, 5 } )
	...
	If !(Empty(cAltSolic)) .And. __cUserId $ cAltSolic
			...
			AAdd(aRotina,{"Alterar Solicitante" , 'U_F0100414'   , 0, 5 } )
	...
	if lFilSimp
				...
				AAdd(aRotina,{"Upload de SP's" , 'U_XRDCAR99'   , 0, 3 } )
	...
	//ADD OPTION aRotina TITLE "Imprime Recusas" ACTION 'MsgRun("Gerando relatório...", "Rel. Recusas", {||U_F0100REC() })'     OPERATION 3  ACCESS 0

Return aRotina
/*/{Protheus.doc} xFILSF
Rotina responsável por mudar o modelo simplificado ou full.
@type function
@version  P12
...
Static Function ViewDef()
	...
	oStruSC7p:AddField("C7_XDBAUT", "24", "Deb. Automatico", "Deb. Automatico", , "C", "@!", ,         , .T., "01", , {"2=Não", "1=Sim"},1 , , .F., , ,  )

	oStruSC7p:AddField("C7_XVLRET", "25", "Ret. Contratual"               , "Ret. Contratual"              , , "N", "@E 99,999,999,999.99", , ""     , .T., "01", , , , , .F., , ,  )
	oStruSC7p:AddField("C7_XCNPJ", "26", "CNPJ Tomador", "CNPJ Tomador", , "C", "@!", , "EMCNPJ"     , .T., "01", , , , , .F., , ,  )
	oStruSC7p:AddField("C7_XNOMFIL", "27", "Nome da Filial", "Nome da Filial", , "C", "@!", , ""     , .F., "01", , , , , .F., , ,  )
	oStruSC7p:AddField("C7_XPORTAD", "28", "Bco Portador", "Bco Portador", , "C", "@!", , "SA6DV"     , .T., "01", , , , , .F., , ,  )
	oStruSC7p:AddField("C7_XAGEPOR", "29", "Agência", "Agência", , "C", "@!", , ""     , .F., "01", , , , , .F., , ,  )
	oStruSC7p:AddField("C7_XDVAPOR", "30", "DV Agência", "DV Agência", , "C", "@!", , ""     , .F., "01", , , , , .F., , ,  )
	oStruSC7p:AddField("C7_XCONPOR", "31", "Conta", "Conta", , "C", "@!", , ""     , .F., "01", , , , , .F., , ,  )
	oStruSC7p:AddField("C7_XDVCPOR", "32", "DV Conta", "DV Conta", , "C", "@!", , ""     , .F., "01", , , , , .F., , ,  )
	...
	Begin Sequence
		...
		If Posicione("SA2",1,xFilial("SA2")+M->C7_FORNECE+M->C7_LOJA,"SA2->A2_XDTFIX") == "1"
			If !lChange
				...
				EndIf
			EndIf
				If cEdit <> "Não" .And. cEdit <> ""
				If nDiasMax < DateDiffDay(dDataEmis,  dDataUtil) 
					Help( , , 'HELP', , "A de vencimento informada passa do limite de " +cValToChar(nDiasMax)+" dias a partir da data de emissão.", 1, 0 , , , , , , {"Verifique o parâmetro FS_DVECTO."})
					Break
				EndIf
			EndIf
		EndIf
...
Return cRet
//Função When do campo C7_XDBAUT
...
/*/{Protheus.doc} fDesCNPJ
Pega descrição da empresa pelo CNPJ
@type function
@version P122410 
@author Ricardo Junior
@since 11/28/2025
@return variant, return_description
/*/
User Function fDesCNPJ()
	Local cRet := ""

	aFiliais := FWLoadSM0()
	//aScan(aFiliais, {|x| x == "M0_CGC"})
	nPos := aScan(aFiliais, {|x| AllTrim(x[18]) == AllTrim(M->C7_XCNPJ) })
	if nPos > 0
		cRet := aFiliais[nPos][7]
	endif
Return cRet
/*/{Protheus.doc} fvldport
Valida when campo portador
@type function
@version P122410 
@author Ricardo Junior
@since 11/28/2025
@return variant, valida
/*/
User Function fvldport()
	Local cGetGrp 	:= SuperGetMV("FS_GRPPORT",,"000000")	
	Local cCodUsr   := RetCodUsr()
    Local cNomUsr   := UsrRetName(cCodUsr)
    Local aGrupos   := UsrRetGrp(cNomUsr, cCodUsr)
	Local lRet 		:= .F.
	
	nPos := aScan(aGrupos, {|x| Alltrim(x) $ AllTrim(cGetGrp) })
...
/*/{Protheus.doc} fvlCNPJ
Valida o CNPJ
@type function
@version P122410  
@author Ricardo Junior
@since 12/1/2025
@return variant, Valida ou não valida
/*/
User Function fvlCNPJ()
	Local lRet := .T.	
	aFiliais := FWLoadSM0()
	//aScan(aFiliais, {|x| x == "M0_CGC"})
	nPos := aScan(aFiliais, {|x| AllTrim(x[18]) == AllTrim(M->C7_XCNPJ) })
