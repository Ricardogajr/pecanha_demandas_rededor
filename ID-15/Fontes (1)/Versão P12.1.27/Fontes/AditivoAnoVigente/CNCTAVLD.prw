...
************************************************
   ...
   While ZVK->(!Eof()) .And. ZVK->ZVK_CODEXT == cCodigo
         ...
         If ZVK->ZVK_TIPDES <> SX3->X3_TIPO
            Aadd(aLogs,{GR_ERR,cField,TP_TYPEDIF,U_FmtStr('Campo "{1}", tipo ("{2}") está diferente no dicionário ("{3}")!',{cField,ZVK->ZVK_TIPDES,SX3->X3_TIPO})})
         Endif

         If GetdToVal(ZVK->ZVK_TAMDES) <> SX3->X3_TAMANHO
            Aadd(aLogs,{GR_ERR,cField,TP_TAMANHO,U_FmtStr('Campo "{1}", tamanho ("{2}") está diferente no dicionário ("{3}")!',{cField,GetdToVal(ZVK->ZVK_TAMDES),SX3->X3_TAMANHO})})
         Endif
         
         If ZVK->ZVK_DECDES <> SX3->X3_DECIMAL
            Aadd(aLogs,{GR_ERR,cField,TP_DECIMAL,U_FmtStr('Campo "{1}", decimal ("{2}") está diferente no dicionário ("{3}")!',{cField,ZVK->ZVK_DECDES,SX3->X3_DECIMAL})})
         Endif
   ...
   While SX3->(!Eof()) .And. SX3->X3_ARQUIVO == cAlias
         ...
         If SX3->X3_CONTEXT == "V" .OR. ( "_XMIGLT" $ cField )
            SX3->(DbSkip())
            Loop
         Endif
...
****************************************                                        
      ...
      For nX := 1 To Len(aOccurs)
          ...
          Do Case
             Case aOccurs[nX,3] == TP_TYPEDIF
                  Aadd(aScript,U_FmtStr("UPDATE "+RetSqlName("ZVK")+" SET ZVK_TIPDES='{1}' WHERE D_E_L_E_T_=' ' AND ZVK_CODEXT='{2}' AND ZVK_CPODES='{3}'" ,{SX3->X3_TIPO,cCodigo,cField}))
             Case aOccurs[nX,3] == TP_TAMANHO
                  Aadd(aScript,U_FmtStr("UPDATE "+RetSqlName("ZVK")+" SET ZVK_TAMDES='{1}' WHERE D_E_L_E_T_=' ' AND ZVK_CODEXT='{2}' AND ZVK_CPODES='{3}'" ,{SX3->X3_TAMANHO,cCodigo,cField}))
             Case aOccurs[nX,3] == TP_DECIMAL
                  Aadd(aScript,U_FmtStr("UPDATE "+RetSqlName("ZVK")+" SET ZVK_DECDES ={1}  WHERE D_E_L_E_T_=' ' AND ZVK_CODEXT='{2}' AND ZVK_CPODES='{3}'" ,{SX3->X3_DECIMAL,cCodigo,cField}))
   ...
   While SX3->(!Eof()) .And. SX3->X3_ARQUIVO == cAlias
         ...
         ZVK->ZVK_DESCDE    := SX3->X3_DESCRIC
         ZVK->ZVK_TIPDES    := SX3->X3_TIPO
         ZVK->ZVK_TAMDES    := cValToChar(SX3->X3_TAMANHO)    
         ZVK->ZVK_DECDES    := SX3->X3_DECIMAL
         ZVK->ZVK_OBRDES    := If(X3Obrigat(cField),"S","N")
         ZVK->ZVK_PREDES    := SX3->X3_F3
         ZVK->ZVK_VIRTUA    := "N"
         ZVK->ZVK_CBOXDE    := SX3->X3_CBOX
         ZVK->ZVK_GRPSXG    := SX3->X3_GRPSXG
         ...
         Else
            ...
            ZVK->ZVK_VALIDA    := SX3->X3_VLDUSER 
            ZVK->ZVK_VLDPRO    := SX3->X3_VALID
            ZVK->ZVK_RELACA    := SX3->X3_RELACAO 
         Endif 
         ZVK->(MsUnLock("ZVK"))
