...
Static Function CriaTMP(cCposTMP,lNMenu,cNmTmpTb)
	...
	If !MsFile(cNmTmpTb)//len(U_ListTab(cNmTmpTb)) == 0
		...
		If _RetExec < 0 
			...
			_RetExec = TCSQLERROR()
			APMsgAlert(AllTrim(_RetExec),"N„o foi possÌvel criar a tabela, a rotina n„o continuar· sendo executada!")
			lCriou	:= .F.	
		Else
			...
			if _RetExec < 0 
				_RetExec = TCSQLERROR()
				APMsgAlert(AllTrim(_RetExec),"N„o foi possÌvel criar a sequÍncia, a rotina n„o continuar· sendo executada!")
				lCriou	:= .F.
			EndIf
