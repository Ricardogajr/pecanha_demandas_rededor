...
Static lValPosto	:= .T. //.T. Indica que o posto é válido, portanto possui vaga disponével
...
Static cClasVlA		:= GetMV('MV_ITMCLVL')//1 é Utiliza Item Contébil e Classe de Valores.,2 é Não Utiliza Item Contébil e Classe de Valores.
Static aMotivo		:= {}
Static TSRA	 

/*{Protheus.doc} F0500401
Movimentaéoes de Pessoas - centralizador de solicitações tipo: Alteração Salarial, Cargo, Carga Horária, Turno e/ou Transferência

@author Cris
@since 20/10/2016
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${return}, ${não hé}
...
User Function F0500401()
	...
	Local aSeek			:= {}
	Local aFieFilter	:= {} 
	Local cVeSal		:= GetMV("MV_XVESAL")		// Siqueira 03/04/24                    
	...
	If !( cAliasSra )->( EOF() )
		...
		_cCmp += "RA_NUMCP|RA_SERCP|RA_UFCNH|RA_MSBLQL| RA_TIPAMED|RA_TPASODO|RA_ASODONT|RA_NUMINSC|RA_SERVICO|RA_CODIGO| RA_ADICEDI|RA_XSTMVTO |RA_SALARIO"		// Siqueira 03/04/24		
	...
	If(RetCodUsr() $ cVeSal)		// Siqueira 03/04/24
		ADD COLUMN oColumn DATA {|| RA_SALARIO} TITLE 'Salário' 	SIZE TamSX3('RA_SALARIO')[1] Picture "@E 999,999.99"  OF oBrowse
	EndIF
	...
	ADD COLUMN oColumn DATA {|| RA_COMPSAB} TITLE 'Comp. Sébado' 	SIZE TamSX3('RA_COMPSAB')[1]  OF oBrowse
...
Static Function MenuDef(cAliasSra,oBrowse)
	...
	Aadd(_aRotina, {"Incluir"    , "U_X0500401(@oBrowse)"        , 0, 3})
	Aadd(_aRotina, {"Visualizar Solicitação"    , "U_F0500412('" + cAliasSra + "')"        , 0, 1})
	Aadd(_aRotina, {"Cancelar Solicitação"    , "U_F0500425('" + cAliasSra + "')"        , 0, 6})
...
Static Function  ModelDef()
	...
	Local bPostMod	:= {||               GF00504VL() }//Valida se foi informado alguma alteração e se as informaéoes  séo validas. // gus
//	Local bPostMod	:= {|| GFSVLSALP() ,  F00504VL() }//Valida se foi informado alguma alteração e se as informaéoes  séo validas. // gus
	Local oModel
	...
	oModel:GetModel('RB6DETAIL'):SetOnlyQuery(.T.)//Indica que o modelo é somente consulta e nao gravével
	oModel:GetModel('RB6DETAIL'):SetNoDeleteLine(.T.)
	oModel:GetModel('TMPDETAIL'):SetOptional( .T. )
	oModel:GetModel('TMPDETAIL'):SetOnlyQuery(.T.)//Indica que o modelo é somente consulta e nao gravével
	...
	//Verifica se jé existe solicitação para o funcionário posicionado e se a mesma jé foi atendida
	oModel:SetVldActivate({|| FS05EXIS() .AND. VldPermis() .And. SetStatic()})//SRA->RA_FILIAL,SRA->RA_MAT
	
Return oModel

/*{Protheus.doc} MDStrCab
Monta os campos da estrutura cabeéalho
@type Static function
@author Cris
@since 24/10/2016
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${oStruct}, ${Modelo do cabeéalho}
*/
Static Function MDStrCab()
	...
	oStruct:AddField("Més/Ano Solicitação"						        ,;	// 	[01]  C   Titulo do campo
	                 "Més/Ano Solicitação"								,;	// 	[02]  C   ToolTip do campo
...
@param lValTp, ${légico}, (Retorna se o motivo  informado é valido ou não)
...
Static Function MDGatilho(nMyOpcao)
	...
	//Valor Reajuste
	// ticket né 5594755 - 415966 - Paulo Dias - ajuste no gatilho.
...
Static Function MDStrTur()
	...
	oStruct:AddField("Descriéao do Turno"					            ,;	// 	[01]  C   Titulo do campo
	                 "Descriéao do Turno"							    ,;	// 	[02]  C   ToolTip do campo
...
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${oStruct}, ${Retorna as caracteristicas e regras do sub-modelo da aba Tansferáncia}
*/
Static Function MDStrTrf()
	...
	oStruct:AddField("Descrição do Centro de Custo"				        ,;	// 	[01]  C   Titulo do campo
	                 "Descriéao do Centro de Custo"						,;	// 	[02]  C   ToolTip do campo
...
//	Private oView // gus
	...
	// Aloca as Views nas suas respectivas éreas
...
Estrutura do cabeéalho a ser visualizado
@type Static function
@author Cris
@since 25/10/2016
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${oStruct}, ${Estrutura do cabeéalho a ser visualizado}
*/
Static Function VWStrCab()
	...
	oStruct:AddField("RH3_MAT"								            ,;	// [01]  C   Nome do Campo
	                 ...
	                 NIL											    )	// [18]  L   Indica pulo de linha apés o campo
	...
	oStruct:AddField("RH3DTSOLI"							            ,;	// [01]  C   Nome do Campo
	                 "03"										        ,;	// [02]  C   Ordem
	                 "Més/Ano Solicitação"      						,;	// [03]  C   Titulo do campo
	                 "Més/Ano Solicitação"		        				,;	// [04]  C   Descricao do campo
	                 NIL								     			,;	// [05]  A   Array com Help
	                 "C"									    		,;	// [06]  C   Tipo do campo
	                 NIL										    	,;	// [07]  C   Picture
	                 NIL    											,;	// [08]  B   Bloco de Picture Var
...
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${lValSal}, ${.T. informações de salario vélidas .F. informações não vélidas}
*/
...
//    MDGatilho(2) // gus
	...
	if nValSal <> 0
		...
		//Efetuar diversar validaéées de acordo com a EF
		if SRA->RA_SALARIO <> nValSal
			...
			If SRA->RA_SALARIO > nValSal
				...
				Else
					...
					If nValSal < nValReal
						Help("",1, "Help", "Validação de Salário(FSVLSALP_04)", "Salário não pode ser menor que " + Transform(nValReal, "@E 999,999.99" ) + " devido carga horária informada!" , 3, 0)
						lValSal	:= .F.
					EndIf
				EndIf
			Else
				If oModAtu:GetModel("HORDETAIL"):GEtValue("HMESPROP") <> 0
					...
					If nValSal < nValReal
						Help("",1, "Help", "Validação de  Salário(FSVLSALP_05)", "Salário não pode ser menor que " + Transform(nValReal, "@E 999,999.99" ) + " devido carga horária informada!" , 3, 0)
						lValSal	:= .F.
					EndIf 
				EndIf
			EndIf
		...
		if cCodCarg <> oModAtu:GetModel("CARDETAIL"):GEtValue("TMP_CARGO")
			...
			if SQ3->(dbSeek(FwxFilial("SQ3")+cCodCarg))
				...
				if  FieldPos("Q3_MSBLQL") > 0 .AND. SQ3->Q3_MSBLQL == '1'
					...
					Help("",1, "Help", "Validação da Cargo/Função(FSVLCARP_02)", "Cargo bloqueado para uso, contate o responsével pela manutenção de cadastro de Cargos!" , 3, 0)
					lValCar	:= .F.
				...
				Else
					...
					if SRJ->(dbSeek(FwxFilial("SRJ")+cCodCarg))
						...
						if  FieldPos("RJ_MSBLQL") > 0 .AND. SRJ->RJ_MSBLQL == '1'
							...
							Help("",1, "Help", "Validação da Cargo/Função(FSVLCARP_04)", "Função bloqueado para uso, contate o responsével pela manutenção de cadastro da função!" , 3, 0)
							lValCar	:= .F.
			...
			if lValCar
				...
				If !IsInCallStack("F00504VL")
					...
					//Verifica se por padréo deve-se  alterar o salário automaticamente
...
o mesmo tipo de solicitação com a quantidade de vagas disponéveis.
@type Static Function
@author Cris
@since 04/11/2016
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${array}, ${nQtdePost= Qtde. disponével,
...
Static Function ValPosto()
	...
	if !(cTabAtu)->(Eof())
		...
		if (cTabAtu)->QTDEDISPONIVEL - U_F0500314(cDepto, cFilNova, cPostNov) <= 0
			If !lLibPst
				lValPosto	:= .F.
			Else
				//MsgAlert("Este posto esté ocupado/reservado. A solicitação será processada, mas o colaborador será movimentado somente apés a desocupação do posto.")
				MsgAlert("Este posto esté ocupado. Caso não realize a adequação, a quantidade de colaboradores existentes será maior que a quantidade prevista.")
				lValPosto	:= .T.
			EndIf			
		Else
	...
	if !lValPosto
		//Help("",1, "Help", "Validação da Posto(ValPosto_01)", "Para o cargo "+cCodCarg+", departamento "+cDepto+" e filial "+cFilAt+" não existe posto disponével!" , 3, 0) 7780516 - Thais Paiva
		_cMsgRet := "Validação da Posto(ValPosto_01) - Para o cargo "+cCodCarg+", departamento "+cDepto+" e filial "+cFilAt+" não existe posto disponével!"
	EndIf
...
@param lValCar, ${légico}, (.T. válido e .F. não válido)
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${return}, ${não hé}
...
Static Function AtCarSal(nSalSQ3,lValCar)
	...
	if cAlCgSlA == 'S'
		...
		if !(oModAtu:GetModel("SALDETAIL"):SetValue("SALARIOPROP",nSalSQ3))
			...
			Help("",1, "Help", "Validação da Cargo/Função/Salário(AtCarSal_01)",;
				"Não foi possével alterar a aba Alteração Salarial!"+;
	...
	Else
		...
		//Monto tabela salarial temporária e se aplicével atualiza o campo salário  proposto
...
@author Cris
@since 03/11/2016
@param oModAtu, objeto, (Descrição do parémetro)
@version P12.1.7
@Project MAN0000007423039_EF_004
...
Static Function MtSALHor(oModAtu)
	...
	//Somente se a carga horária foi alterada e for diferente da Carga horaria padréo 220
	if nCrgHrPp <> 0 .AND. nCrgHrPp <> nCrgHrAt .AND. nCrgHrPp <= 220
		...
		if nFxTabAt > 0 .AND. oModAtu:GetModel("RB6DETAIL"):GetValue("VALORATU",1) <> 0
			...
			//Busco o percentual de toleráncia
...
Static Function FSVLHORP(nOpc)
	...
	if !Empty(cCargHor)
		...
		Elseif  oModAtu:GetModel("HORDETAIL"):GEtValue("HMESPROP") < 0
			...
			Help("",1, "Help", "Validação da Carga Horária(FSVLHORP_07)",+;
				" Valor més negativo não é permitido." , 3, 0)
			lValHor	:= .F.
	...
	if !Empty(cCargSem)
		...
		Elseif oModAtu:GetModel("HORDETAIL"):GEtValue("HSEMPROP") < 0
			...
			Help("",1, "Help", "Validação da Carga Horária(FSVLHORP_05)",+;
				" Valores negativos não séo permitidos." , 3, 0)
			lValHor	:= .F.
	...
	If lValHor .And. nOpc == 1
		AVISO("Aviso", "Para os casos em que houver ajuste no salário hora, analisar o campo salário proposto na aba éAlteração Salarialé", { "Ok"}, 1) 
	EndIf
	
Return lValHor

/*{Protheus.doc} FSVLTURP
Vélida os dados da  Troca de Turno
@type Static function
@author Cris
@since 31/10/2016
@version P12.1.7
...
Static Function FSVLTURP()
	...
	if !Empty(cFilDes) .AND. len(Alltrim(cFilDes)) > 4 //4 primeiras posiéées: Empresa+unidade de negécio
					...
					if oModAtu:GetModel("HORDETAIL"):GetValue("HMESPROP") == 0
						...
						if ! MsgYesNo("A carga horária de ("+Transform(oModAtu:GetModel("HORDETAIL"):GetValue("TMP_H_M_AT"),"@ 999.9999")+") será mantida?","Atenção")
							...
							if !(oModAtu:GetModel("HORDETAIL"):SetValue("HMESPROP",oModAtu:GetModel("HORDETAIL"):GetValue("TMP_H_M_AT")))
								...
								Help("",1, "Help", "Atualização Automética(FSVLTURP_04)","Não foi possével atualizar a Carga Horária a informe manualmente." , 3, 0)
...
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${lValTrf}, ${.T. a transferência esta valida e .F. não esta vélida}
*/
Static Function FSVLTRFP(cCampo)
	...
	//Bloqueia a Solicitação de uma  transferencia com a alteraéaé de salário.
		...
		If !Empty(cCCTrf)
			...
			Else
				...
				If CTT->CTT_CLASSE == '1'
					...
					Help("",1, "Help", "Validação da Transferência(FSVLTRFP_06)", "Selecione um centro de custo analético!" , 3, 0)
					lValTrf	:= .F.
				Else
					lValTrf	:=	.T.
				EndIf
...
Static Function FS05EXIS()
	...
	If !Empty(aMatInc[02])
		...
		Else
			Help("",1, "Help", "Atenção", "Usuério não tem permisséo para realizar movimentação para si préprio." , 3, 0)
			lNExiSol := .F.
		EndIf
...
validação total no botéo Confirmar
@type Static function
@author Cris
@since 28/10/2016
@version P12.1.7
...
Static Function F00504VL()
	...
	Local lHorProp	:= FSVLHORP() // ticket né 2669524 [AMS] - Paulo Dias - 11/05/2018
	...
	//possével cenério: quando existir transferência
	...
	//Verifico se existe solicitação de posto em aberto e  se existe se o némero de solicitaéoes ultrapassa o numero disponivel de posto
		...
		Else
			...
			//Fim - 7780516 - Thais Paiva
				If !Empty(oModAtu:GetModel("TRFDETAIL"):GEtValue("FILIPROP")) .OR. !Empty(oModAtu:GetModel("TRFDETAIL"):GEtValue("DEPTOPROP")) .OR. !Empty(oModAtu:GetModel("TRFDETAIL"):GEtValue("POSTOPROP"))
					cSalVali  := "Não existe posto disponével para as informações de alteração."
					lSolicVal := .F.
				EndIf
...
@author Cris
@since 04/11/2016
@param oModel, objeto, (Descrição do parémetro)
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${légico}, ${.T. gravado com sucesso .F. não gravado}
*/
Static Function PrepGrv(oModAtu)
	...
	//Preparando o cabeéalho da solicitação
	...
	If len(aCabRH3) > 0 .AND. len(aCampos) > 0 .AND. len(aCampos) == len(aDados)
		...
		if RH3->(dbSeek(SRA->RA_FILIAL+cNumSol+SRA->RA_MAT))
			...
			Aviso("Solicitação","Némero da solicitação:"+RH3->RH3_CODIGO ,{'OK'},1)
	...
	if !Empty(cNumSol)
		...
		if SRA->(dbSeek(RH3->RH3_FILIAL+RH3->RH3_MAT))
			...
			if SRA->(Reclock("SRA",.F.))
				...
				SRA->(MsUnlock())
			Else				
				Help("",1, "Help", "Status Funcionário(PrepGrv_01)", "Não foi possével sinalizar no cadastro de funcionário que o  mesmo possui solicitação em aberto!" , 3, 0)				
			EndIf
	...
	lETurPrp	:= .F.
	lETrfPrp	:= .F.
	lValPosto	:= .T. //.T. Indica que o posto é válido, portanto possui vaga disponével
	cSubGrp		:= ''  //Codigo da viséo que será gravada
...
Static Function PrepDds(aCpoRH3,cEmailRe)
	...
	If aRetSup[1][1] //Encontrou aprovador
		...
		If (aRetSup[1][5] == "FM" .And. cNotif == "2") .Or. (aRetSup[1][5] == "FM" .And. ("aprova direto" $ aRetSup[1][8] .OR. "Terminou a estrutura da viséo!" $ aRetSup[1][8] ))
			cStatus := "4"
		Else
			cStatus := "1"
		EndIf
...
(Seleciona a viséo conforme aba(s) alterada(s)
@type Static function
@author Cris
@since 31/10/2016
@version P12.1.7
...
@author Cris
@since 31/10/2016
@param @aCampos, ${param_type}, (Descrição do parémetro)
@param @aDados, ${param_type}, (Descrição do parémetro)
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${return}, ${não hé}
*/
...
@author Cris
@since 31/10/2016
@param aCampos, array, (Descrição do parémetro)
@param aDados, array, (Descrição do parémetro)
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${return}, ${não hé}
*/
...
Static Function MntEmail(cCodRH3,cMatAtu,cNomeFun,cEmailRe)
	...
	cBody += "Némero da Solicitação:"+cCodRH3+CRLF
	cBody += '</pre></body></html>'
	
	//FWMsgRun(,{|| U_F0200304(cAssunto, cBody, cEmailRe)},"Aguarde...","Estabelecendo conexéo com o serviéo de e-mail." )
	FWMsgRun(,{|| lEnvEmail:= EnvEmail("",cAssunto,cBody,cEmailRe,"")},"Aguarde...","Estabelecendo conexéo com o serviéo de e-mail." )
	...
	Else
		...
		Aviso("INSUCESSO - Email","E-mail NéO enviado. Comunique aos envolvidos!" ,{'OK'},1)
...
@param cTo, character, (endereéo de Destino do e-mail)
@param cCC, character, (endereéo copia de destino)
@version P12.1.7
@Project MAN0000007423039_EF_004
@return ${return}, ${não hé}
*/
Static Function EnvEmail(cArquivo,cSubject,cBody,cTo,cCC)
	...
	If !lOK
		...
		conout("F0500401: Falha na Conexéo com Servidor de E-Mail")
		//Aviso('Email não enviado',"Falha na Conexéo com Servidor de E-Mail. ",{'OK'},3)
		
	Else
		
		If lAutentica
			If !MailAuth(cUserAut,cPassAut)
				//Aviso('Email não enviado',"Falha na Autenticação do Usuério. ",{'OK'},3)
...
Valida se o usuário tem permisséo para realizar movimentação
@author		Cris
@since		09/11/2016
@version	P12.1.7
@Project	MAN0000007423039_EF_004
...
Static Function VldPermis()
	...
	If !lRet
		Help("",1, "Help", "Atenção", "Usuério não tem permisséo para realizar movimentação para esse funcionário" , 3, 0)
	EndIf
...
Static Function SetStatic()
	...
	cClasVlA		:= GetMV('MV_ITMCLVL')//1 é Utiliza Item Contébil e Classe de Valores.,2 é Não Utiliza Item Contébil e Classe de Valores.
	aMotivo			:= {}
Return .T.
...
User Function F0500425(cAliasSra)
	...
	If lSolicita
		...
		If !Empty(aUser[1]) .AND. !Empty(aUser[2])
			...
			Else
				MsgAlert("Usuério não tem permisséo para cancelar a solicitação")
			EndIf
		EndIf
	Else
...
If !Empty(cPostTrf)	
	...
	ElseIf (RCL->RCL_NPOSTO <= (RCL->RCL_OPOSTO + nTotal)) 
		If !lLibPst
			_cMsgRet := "Validação da Transferência(VLDPOSTOD_5) - Existem solicitações vinculadas a esse posto, Favor verificar."
		EndIf
	Else
		//Se o Cargo Do Posto proposto for diferente
		If Empty(cCargPrp)
	...
	EndIf
Else
	_cMsgRet := "Validação da Transferência(VLDPOSTOD_7) - Atenção: é obrigatório preencher o campo POSTO!"  //ticket né 9203072 -- cria validação para preenchimento obrigatório do campo POSTO
EndIf
