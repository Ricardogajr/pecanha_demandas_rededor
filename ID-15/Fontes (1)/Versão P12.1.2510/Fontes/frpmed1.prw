...
oTempTable:AddIndex("1", {"CR_FILIAL"} )
oTempTable:AddIndex("2", {"CR_NUM"} )
oTempTable:AddIndex("3", {"CR_TIPO"} )
oTempTable:AddIndex("4", {"CR_TIPO","CR_TIPO","CR_NUM"} )
oTempTable:AddIndex("5", {"TR_PLANRP"} )
...
Static Function MenuDef()
	...
	Aadd(aOpcVis , {"Formulário da aprovação"    , 'U_XXRPFORM()'        , 0, 2})
	Aadd(aOpcVis , {"Documento emitido"    , 'U_XFDOCRPE()'        , 0, 2})
	Aadd(aOpcVis , {"Saldo para aprovação"    , 'u_XfSldARPM()'        , 0, 2})
	...
	Aadd(aRotina , {"Rel. Aprovações"    , 'RptStatus({|| u_fProcRP(010)}, "Aguarde...", "Processando...")'        , 0, 4})
	Aadd(aRotina , {"Visualizar"    , aOpcVis        , 0, 2})
	Aadd(aRotina , {"Bloquear"    , 'RptStatus({|| u_fProcRP(002)}, "Aguarde...", "Processando...")'        , 0, 4})
	Aadd(aRotina , {"Estornar"    , 'RptStatus({|| u_fProcRP(003)}, "Aguarde...", "Processando...")'	        , 0, 5})
	Aadd(aRotina , {"Pendencias de subordinados"    , 'U_PNDSBRP()'        , 0, 3})
	Aadd(aRotina , {"Superior"    , aOpcSup        , 0, 4})
	Aadd(aRotina , {"Banco específico"    , aOpcBco        , 0, 4})
