...
// 19/08/2025 - Thais Paiva - Fonte ajustado após reversão - Release 2410 - 24492226
*/
User Function F0500402(oModel)
	...
	Private cConPeriod	:= .F.	//Verifica se existe a nova consulta períodos para o GPEA180 - Thais Paiva 24528651
	Private cConNrPag	:= .F. //Verifica se existe a nova consulta número de pagamento para o GPEA180 - Thais Paiva 24528651
	...
	If SRA->(DbSeek(cFilFun + cMatFun))
		...
		For iCpoSRA	:= 1 to len(aCpos)
			...
			if Posicione("SX3", 2, Alltrim(aCpos[iCpoSRA][1]), "X3_TIPO") == "N"
				&(aCpos[iCpoSRA][1])	:= GetDToVal(aCpos[iCpoSRA][2]) //Val(aCpos[iCpoSRA][2]) - Thais Paiva - 19680839
			Else
		...
		For iCpoCg := 1 to len(aCpCarg)
			if Posicione("SX3", 2, Alltrim(aCpCarg[iCpoCg][1]), "X3_TIPO") == "N"
				&(aCpCarg[iCpoCg][1])	:= GetDToVal(aCpCarg[iCpoCg][2]) //Val(aCpCarg[iCpoCg][2]) - Thais Paiva - 19680839
...
Static Function TCFA040Atende( oModel, cStatus, dDtAtu )
	...
	Private lEmpDif	        	:= .F. //Alexandre Felici0  Ticket 26084000
	Private cConSM0		:= .F. //Thais Paiva - 26044402
	...
	cCadastro               	:= OemToAnsi( "Transferências" )  //"TransferË†ncias"
	cTipSolicPortal         	:= "4"
