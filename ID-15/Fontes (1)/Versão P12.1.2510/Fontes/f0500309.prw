...
User Function F0500309()
	...
	if !Empty(cAuthWS) //Início Thais Paiva - 24273898
		oInf:_HEADOUT :=  { "Authorization: BASIC "+ ENCODE64(rc4crypt( cAuthWS ,"AuthWS#ReceiptID", .F.,.T.)) }
	...
	Case cTipo = '002'
		...
		oSol := WSW0500308():New()
		if !Empty(cAuthWS) //Início Thais Paiva - 24273898
			oSol:_HEADOUT :=  { "Authorization: BASIC "+ ENCODE64(rc4crypt( cAuthWS ,"AuthWS#ReceiptID", .F.,.T.)) }
		endif//Fim Thais Paiva - 24273898
		...
		If oSol:oWSBscSolVagsRESULT:cRet == ".T."
			...
			//cJusVaga   := ALLTRIM(oSol:oWSBscSolVagsRESULT:cJusVaga)
			cJusVaga     := IIF(AllTrim(oSol:oWSBscSolVagsRESULT:cJusVaga) == "1", "Aumento de Quadro", IIF(AllTrim(oSol:oWSBscSolVagsRESULT:cJusVaga) == "2", "Substitução de Colaborador", IIF(AllTrim(oSol:oWSBscSolVagsRESULT:cJusVaga) == "3", "Substitução de Afastado", "Ajuste de Estrutura")))
			cSolComp     := IIF(AllTrim(oSol:oWSBscSolVagsRESULT:cSolComp) == "1", "Sim", "Não")
			//cSolComp   := ALLTRIM(oSol:oWSBscSolVagsRESULT:cSolComp)
			cLocali := IIF(AllTrim(oSol:oWSBscSolVagsRESULT:cLocali) == "1", "Passeio", IIF(AllTrim(oSol:oWSBscSolVagsRESULT:cLocali) == "2", "Plataforma", IIF(AllTrim(oSol:oWSBscSolVagsRESULT:cLocali) == "3", "Central de Atendimento", "Outros")))
			//cLocali   := ALLTRIM(oSol:oWSBscSolVagsRESULT:cLocali)
			cXOutros   := ALLTRIM(oSol:oWSBscSolVagsRESULT:cXOutros)
	...
	Case cTipo = '008'
		...
		if !Empty(cAuthWS) //Início Thais Paiva - 24273898
			oInfLog:_HEADOUT :=  { "Authorization: BASIC "+ ENCODE64(rc4crypt( cAuthWS ,"AuthWS#ReceiptID", .F.,.T.)) }
		endif//Fim Thais Paiva - 24273898
		...
		if !Empty(cAuthWS) //Início Thais Paiva - 24273898
			oWSFerProg:_HEADOUT :=  { "Authorization: BASIC "+ ENCODE64(rc4crypt( cAuthWS ,"AuthWS#ReceiptID", .F.,.T.)) }
		endif//Fim Thais Paiva - 24273898
		...
		if !Empty(cAuthWS) //Início Thais Paiva - 24273898
			oFerObj:_HEADOUT :=  { "Authorization: BASIC "+ ENCODE64(rc4crypt( cAuthWS ,"AuthWS#ReceiptID", .F.,.T.)) }
		endif//Fim Thais Paiva - 24273898
