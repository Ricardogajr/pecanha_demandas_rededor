            ...
            If cStatus == "true"
                ...
                While !SM0->(EOF())
                    ...
                    If AScan(aTabEmp, RetSQLName("SE2")) == 0
                        ...
                        lIntMnk := SuperGetMV("MK_INTMNK", , .F.)
                            ...
                            */
                            ...
                            While !(cAliasTRB)->(EOF())
                                ...
                                */
                                ...
                                If Empty((cAliasTRB)->E2_PARCIR) .And. (cAliasTRB)->E2_IRRF >= GetMV("MV_VLRETIR")
                                    nValor := nValor - (cAliasTRB)->E2_IRRF
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCINS) .And. (cAliasTRB)->E2_INSS >= GetMV("MV_VLRETIN")
                                    nValor := nValor - (cAliasTRB)->E2_INSS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCISS) .And. (cAliasTRB)->E2_ISS >= GetMV("MV_VRETISS")
                                    nValor := nValor - (cAliasTRB)->E2_ISS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCPIS) .And. (cAliasTRB)->E2_PIS >= GetMV("MV_VRETPIS")
                                    nValor := nValor - (cAliasTRB)->E2_PIS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCCOF) .And. (cAliasTRB)->E2_COFINS >= GetMV("MV_VRETCOF")
                                    nValor := nValor - (cAliasTRB)->E2_COFINS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCSLL) .And. (cAliasTRB)->E2_CSLL >= GetMV("MV_VRETCSL")
