...
Param2 := StaticCall(RDILIB02,GetDir,101) + AllTrim(QZ1->QZ1_DESTIN) + "\"
...
If  UPPER(Right(cPathTXT,4)) == ".TXT"
    lContinua := .T.
    aArqsTXT := StaticCall(RDI004,fDirectory,Left(cPathTXT,RAT("\",cPathTXT)),Substr(cPathTXT,RAT("\",cPathTXT)+1),.T.)
ElseIf StaticCall(RDILIB02,fExistDir,cPathTXT)
    lContinua := .T.
    aArqsTXT := StaticCall(RDI004,fDirectory,cPathTXT,Upper(Alltrim(cAliasTAB))+"*.TXT",.T.)
Else
    ...
    If Len(aArqsTXT) > 0
      ...
      If IsInCallStack( 'U_RDI008' ) .OR. MsgYesNo("Confirma a Migração de [ "+Alltrim(Str(Len(aArqsTXT)))+" ] Arquivo(s) TXT ???", "Tabela [ "+cNomTabTMP+" ]")
         ...
         For nTXT := 1 To Len(aArqsTXT)    
             ...
             nTotLinha := StaticCall(RDI004,CountReg,0,aArqsTXT)
         ...
         StaticCall(RDILIB02,MoveRead,aArqsTXT,Strzero(nLote,10))
...
RecLock("QZ3",.t.)
         ...
         QZ3->QZ3_VALIDA  := "S"
         QZ3->QZ3_MAXREC  := StaticCall(RDI004,GetMaxRecno,Upper(Alltrim(cAliasTAB)) )
...
Static Function fCHARGE_R(nTXT)
   ...
   If ! StaticCall(RDILIB02,fExistDir,Upper(cPathTXT+"\LIDOS\"))
        StaticCall(RDILIB02,fMkWrkDir,Upper(cPathTXT+"\LIDOS\"))
   EndIf

   If !(StaticCall(RDILIB02,fExistDir,Upper(cPathTXT+"\LIDOS\"+cPathLido)))
        StaticCall(RDILIB02,fMkWrkDir,Upper(cPathTXT+"\LIDOS\"+cPathLido))
   EndIf
...
If lDic
   ...
   While SX3->(!Eof()) .and. SX3->X3_ARQUIVO == cAlias
     ...
     If SX3->X3_CONTEXT == "V" .or. lInc
        If (cStack == NIL )
            If Type("M->"+Trim(SX3->X3_CAMPO))=='U'
              _SetOwnerPrvt(Trim(SX3->X3_CAMPO),CriaVar(Trim(SX3->X3_CAMPO),lInitPad))
            Else
               &("M->"+Trim(SX3->X3_CAMPO)) := CriaVar(Trim(SX3->X3_CAMPO),lInitPad)
            EndIf
        Else
...
While TMPPARAM->(!Eof())
   aAdd(aValida,;
        ...
        TMPPARAM->QZ2_VALIDA,;
        TMPPARAM->QZ2_CPOORI,;
        StaticCall(RDILIB02,GetDir,101) + AllTrim(TMPPARAM->QZ1_DESTIN) + "\",;
        " "                 ,;
        TMPPARAM->QZ1_CODIGO,;
        ""                  ,;
        0                   ,;
...
If SX3->(DbSeek(cAliasTAB))	
   aCampos := {}
   While SX3->X3_ARQUIVO == cAliasTAB .And. SX3->(!Eof()) 
     If SX3->X3_CONTEXT <> "V"
        aAdd(aCampos,{SX3->X3_ARQUIVO,;
                      SX3->X3_CAMPO,;
                      SX3->X3_TIPO,;
                      SX3->X3_TAMANHO,;
                      SX3->X3_DECIMAL,;
...
Static Function fCHARGE_A()
   ...
   cNomTabTMP := SX2->X2_CHAVE+' - '+Alltrim(SX2->X2_NOME)
...
************************************
    ...
    nHandle   := StaticCall(RDILIB02,OpenFile,cFileName,FO_READ + FO_SHARED)
    If ( nHandle = -1 )
