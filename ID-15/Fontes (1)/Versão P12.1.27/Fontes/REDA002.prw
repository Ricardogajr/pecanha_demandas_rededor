...
Static Function fProcessa(cXIdProc)	
	...
	cQuery += "  cFilial  "+ cTabSC1 +".C1_FILIAL%type;" 	+ CRLF
	...
	cQuery += "     OPEN c1;" 	+ CRLF
	cQuery += "     LOOP" 	+ CRLF
	cQuery += "        FETCH c1 INTO cFilial, cNum, cProduto, cItem; " 	+ CRLF
	cQuery += "        EXIT WHEN c1%notfound;" 	+ CRLF
	cQuery += "          cChaves := cChaves  || RTRIM(cFilial) || RTRIM(cNum) || RTRIM(cItem) || ';'  ; " 	+ CRLF
	...
	If TcSqlExec(cQuery) < 0
		ApMsgInfo("Erro na criação da view (" + cFuncName + ") !")
		Return
	EndIf
	...
	Endif	
	cQuery += " 	GROUP BY C1_FILIAL, C1_PRODUTO, C1_LOCAL, C1_CC, C1_DESCRI,C1_SEGUM,C1_OK,C1_CONDPAG,C1_MOEDA,C1_TPSC,C1_XTPSC,C1_XIDBIO, C1_CC, C1_LOCAL, C1_XIDPROC" 	+ CRLF
	...
	While !(cAliasSC1)->(Eof()) //.And. (cAliasSC1)->C1_FILIAL == xFilial("SC1")
		...
		aAdd(aAux, {{"C1FILIAL", 	(cAliasSC1)->C1_FILIAL},;
					...
					{"XCONDPAG", 	SuperGetMv("MV_XCONDBI",,"1")},;
					{"XDTCOTA", 	DToC(Date() + SuperGetMv("MV_XDTBIO",,1))},;
					{"XHRCOTA", 	SuperGetMv("MV_XHRBIO",,"00:00")},;
