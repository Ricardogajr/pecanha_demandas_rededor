                            ...
                            */
                            ...
                            While !(cAliasTRB)->(EOF())
                                ...
                                */
                                ...
                                If Empty((cAliasTRB)->E2_PARCIR) .And. (cAliasTRB)->E2_IRRF >= GETSPARAM("MV_VLRETIR")
                                    nValor := nValor - (cAliasTRB)->E2_IRRF
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCINS) .And. (cAliasTRB)->E2_INSS >= GETSPARAM("MV_VLRETIN")
                                    nValor := nValor - (cAliasTRB)->E2_INSS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCISS) .And. (cAliasTRB)->E2_ISS >= GETSPARAM("MV_VRETISS")
                                    nValor := nValor - (cAliasTRB)->E2_ISS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCPIS) .And. (cAliasTRB)->E2_PIS >= GETSPARAM("MV_VRETPIS")
                                    nValor := nValor - (cAliasTRB)->E2_PIS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCCOF) .And. (cAliasTRB)->E2_COFINS >= GETSPARAM("MV_VRETCOF")
                                    nValor := nValor - (cAliasTRB)->E2_COFINS
                                EndIf

                                If Empty((cAliasTRB)->E2_PARCSLL) .And. (cAliasTRB)->E2_CSLL >= GETSPARAM("MV_VRETCSL")
