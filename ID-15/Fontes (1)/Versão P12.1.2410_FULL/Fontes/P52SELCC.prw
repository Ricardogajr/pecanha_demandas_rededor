...
Static Function MenuDef()
	...
	//Criação das opções
	ADD OPTION aRotina TITLE 'Salvar'  ACTION 'u_xP52SLCC'     OPERATION 2 ACCESS 0
	ADD OPTION aRotina TITLE 'Fechar'  ACTION 'CloseBrowse()'     OPERATION 2 ACCESS 0
Return aRotina
	...
	While !(cTempTable)->(EoF())
		...
		If oMarkBrowse:IsMark()
			...
			if !P52->(DbSeek(FwxFilial("P52") + cCusto + (cTempTable)->FILIAL))										
				...
				P52->P52_XUSR := UsrRetName(RetCodUsr())
				P52->P52_ATIVO := "1"
				P52->(MsUnlock())
			endif			
		else
