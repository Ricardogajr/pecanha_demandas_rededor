...
Static Function MenuDef()
	...
	Aadd(aRotina , {"Visualizar"    , "VIEWDEF.F0500300"        , 0, 2})
	Aadd(aRotina , {"Incluir"    , "VIEWDEF.F0500300"        , 0, 3})
	Aadd(aRotina , {"Cancelar Solicitação"    , "U_F0500325"        , 0, 6})
...
Local aRetUsr  := {}
	...
	aDados := RetDados()
	//Verifica se a chamado foi pelo admissão digital
	If FwIsInCallStack('U_DOREXCFAP')	
		aRetUsr := U_ValidUsa()
		cIdProt := aRetUsr[3]
	Else
		cIdProt := __cUserId		
	EndIf
	If (PswSeek(cIdProt,.T.))
		aDdSolic := PswRet()			
	...
	Else
		...
		Else
			cGetError := 'F/Não há Vínculo entre o Solicitante e o cadastro de Funcionários.'
		EndIf
		lRet := .F.
	EndIf
...
Static Function RetDados()
	...
	If lFatInt
		aDados := { XFilial("PA2"),;
			...
			SRA->RA_SALARIO,;			
			M->PA2_TPALTS}
			//campos apagados da tabela pela rededor
			/*SRA->RA_CARGO,;
			...
			//(SRA->RA_ADMISSA),;*/
	Else
			aDados := { XFilial("PA2"),; //01
				SQS->QS_FILIAL,; //02
				FWFilialName(,SQS->QS_FILIAL,1),;  //03
				M->PA2_CDVAGA,; //04
				M->PA2_VLVAGA,;  //05
				"EXTERNO",;  //06
				SQS->QS_FILPOST,;  //07
				SQS->QS_POSTO,;  //08
				SQS->QS_FUNCAO,;  //09
				POSICIONE("SRJ",1,XFILIAL("SRJ") + SQS->QS_FUNCAO, "RJ_DESC"),;  //10
				SQS->QS_CC,;  //11
				Posicione("CTT", 1, XFilial("CTT") + SQS->QS_CC, "CTT_DESC01"),;  //12
				Posicione("RCL", 2, SQS->QS_FILPOST + SQS->QS_POSTO, "RCL_DEPTO"),;  //13
				Posicione("SQB", 1, XFilial("SQB") + RCL->RCL_DEPTO, "QB_DESCRIC"),;  //14
				M->PA2_SLFECH,;  //15
				SQS->QS_XTURNO,;  //16
				SQS->QS_XJORN,;  //17
				M->PA2_CDCAND,;  //18
			...
			If SRJ->(DbSeek(XFilial("SRJ")+SQS->QS_FUNCAO))
				If SQ3->(DbSeek(XFilial("SQ3")+SRJ->RJ_CARGO))
					...
					If !(Empty(aAux))
						...
						ElseIf cTip == "E" .And. oMdl:getValue('PA2_SLFECH') > aAux[LEN(aAux)][3] //ACIMA DA FAIXA
							...
							Else
								...
								Else
									cGetError := "F/Não existe orçamento disponível para realizar a Contratação."
								EndIf
								lRet := .F.
					...
					Else
						...
						Else
							cGetError := 'F/Tabela Salarial vínculada ao Cargo não encontrada!'
						EndIf
						lRet := .F.
					EndIf
				Else
					...
					Else
						cGetError := 'F/Cargo vínculado a Função não encontrado!'
					EndIf
					lRet := .F.
				EndIf
			Else
				...
				Else
					cGetError := 'F/Função vínculada a vaga não encontrada!'
				EndIf
				lRet := .F.
			EndIf
		Else
			...
			Else
				cGetError := 'F/Vaga não encontrada!'
			EndIf
			lRet := .F.
		EndIf
			...
			If cTipo == "AP"
				...
				Else
					...
					If !lRet
						...
						Else
							cGetError := "FAP Reprovada, Valor do sálario hora atual maior que o da vaga!"
						EndIf
					EndIf
...
Static Function FiltraPA2()
	...
	Else
		...
		Else
			cGetError := "F/Não existem vagas disponíveis!"
		EndIf
		lRet := .F.
	EndIf
...
Static Function PreVal(oModel)
	...
	If nOpera != 5
		If !Empty(oMdl:getValue('PA2_FILVG'))
			...
			If PA2->(DbSeek(PA2->(XFILIAL("PA2") + oMdl:getValue('PA2_CDVAGA'))))
				while PA2->(PA2_FILIAL+PA2_CDVAGA) == PA2->(XFilial("PA2") + oMdl:getValue('PA2_CDVAGA') )
					If ALLTRIM(PA2->(PA2_SIT)) != "RE"
						...
						Else
							cGetError := 'F/Já existe uma solicitação de FAP em aberto para esta vaga.'
						EndIf
						lRet := .F.
						Exit
					Else
			...
			If lRet .And. !VldCand(xFilial("PA2"),oMdl:getValue('PA2_CDCAND'))
				...
				Else
					cGetError := 'F/O Candidato selecionado já está alocado em uma FAP.'
				EndIf
				lRet := .F.
			EndIf
		Else
			...
			Else
				cGetError := 'F/Filial da Vaga Não Preenchida, faça a Busca da Vaga pelo botão F3'
			EndIf
			lRet := .F.
		EndIf
	EndIf
	If lRet
		...
		If SQG->(DbSeek(XFilial("SQG")+oMdl:getValue('PA2_CDCAND')))		
			If SitCand() .And. Empty(oMdl:getValue('PA2_TPALTS'))
				...
				Else
					cGetError := 'F/Necessário informar o tipo de aumento para FAP Interna'
				EndIf
				lRet := .F.				
			EndIf
		Else
			...
			Else
				cGetError := 'F/Currículo Não Encontrado'
			EndIf
			lRet := .F.
		EndIf				
	EndIf
	...
	//							MsgAlert("Este posto está ocupado. A solicitação será processada, mas o colaborador será movimentado somente após a desocupação do posto.") //Interno
							...
							Else
								cGetError := "T/Este posto está ocupado. Caso não realize a adequação, a quantidade de colaboradores existentes será maior que a quantidade prevista."
							EndIf
						Else
							If !FwIsInCallStack('U_DOREXCFAP')
