...
/*/
//--------------------------------------------------------------------
User Function UPDSD11( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UPDFP09" )
				Else
					MsgStop( "Atualização não realizada.", "UPDFP09" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	14																		, ; //X3_TAMANHO
	2																		, ; //X3_DECIMAL
	'Vr.Retencao'															, ; //X3_TITULO
	'Vr.Retencao'															, ; //X3_TITSPA
	'Vr.Retencao'															, ; //X3_TITENG
	'Vr de Retencao'															, ; //X3_DESCRIC
	'Vr de Retencao'															, ; //X3_DESCSPA
	'Vr de Retencao'															, ; //X3_DESCENG
	'@E 99,999,999,999.99'    												, ; //X3_PICTURE
	''																		, ; //X3_VALID
	'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', ; //X3_USADO
	'POSICIONE("SC7",1, XFILIAL("SC7")+SD1->D1_PEDIDO+"0001","C7_XVLRET")'	, ; //X3_RELACAO
	''																		, ; //X3_F3
	0																		, ; //X3_NIVEL
	'xxxxxx x'																, ; //X3_RESERV
	''																		, ; //X3_CHECK
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDFP09" ) ) ) ;
