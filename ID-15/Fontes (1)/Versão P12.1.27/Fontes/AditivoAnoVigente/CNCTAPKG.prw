...
*********************************
   ...
   If (Select(cAlias) == 0)
      cFilePkg := CriaTrab(aStruct,.T.)
      dbUseArea(.T.,RDDSetDefault(),cFilePkg,cAlias,.F.,.F.)
   Else
      (cAlias)->(__dbZap())
   Endif
...
*********************************
   ...
   For nX := 1 To Len(aFlds)
       ...
       If ! (cAlsSx3)->( dbSeek(PadR(aFlds[nX],nLenSx3)) )
