...
*********************************************
Static Function xBetween(xVar,xValue1,xValue2)
*********************************************
...
*******************************
Static Function fMkWrkDir(cDir)
*******************************
   Local lRet := .T.
...
****************************
   ...
   If ! fExistDir( cRet ) .And. ! fMkWrkDir(cRet)
...
*************************************************
Static Function PathRelat(cPath,cStart,lOnlyPath)
*************************************************
   Local cRet   := cPath
   Local nStart := 0 
...
*********************************
   ...
   If File(cBatTmp)
      If ( WaitRun(cBatTmp) == 0 ) 
         If File(cTxtTmp)
            nHandle := OpenFile(cTxtTmp,FO_READ)
...
**
Static Function Array2Excel(aHeader,aCols,cTitulo)
**************************************************
   Local aDados      := {}
   Local aRow        := {}
   Local nX := nY    := 0
   ...
   oFWMsExcel := FWMSExcel():New()
	  ...
	  For nX := 1 To nHCount
	      oFWMsExcel:AddColumn(cWorkSheet,cTitulo,aHeader[nX],1,1) 
      Next nX
	  
	  For nX := 1 To nDCount
          oFWMsExcel:AddRow(cWorkSheet,cTitulo,aDados[nX])
	  Next

    oFWMsExcel:Activate()
    oFWMsExcel:GetXMLFile(cFileName)
...
***********************************
Static Function SPCompile(cObjName)
***********************************
   Local lRet       := .T.
...
****************************************
   ...
   Local cRet  := StaticCall(RDILIB02,GetDir,201)
...
*****************************************
Static Function FileInCfg(cFileName,cSub)
*****************************************
   Local cFile := ""
   Local cRet  := StaticCall(RDILIB02,GetDir,200)
...
********************************
Static Function RunBat(cFileBat)
********************************
   Local lRet      := .F.
   Local cPath     := GetTmpMigra()   
   ...
   Else
      lRet := ( WaitRun( FileInCfg(cFileName,"\TEMP\") ,1) == 0 )
   Endif
...
********************************
Static Function fExistDir(cPath)
********************************
   Local cAux := PathRelat(cPath)
...
****************************************
   ...
   cFileName := PathRelat(cFileName,,.F.)
   nRet      := FCreate(cFileName,nMode)
   
Return nRet

*****************************************   
Static Function OpenFile(cFileName,nMode)
*****************************************
   Local nRet := 0
...
*******************************************************
Static Function CopyS2T(aMasks,cPathSrv,cPathClt,lMove)
*******************************************************
   Local lRet      := .F.
   Local aAuxFiles := {}
   Local cMask     := "*"
...
*******************************************
Static Function NextNum(cAlias,cField,lSum)
*******************************************
...
*************************************
Static Function EraseAll(cPath,cMask)
*************************************
   Local lRet   := .T.
   Local aFiles := {}
   Local nX     := 0
