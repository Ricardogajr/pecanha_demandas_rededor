...
********************************
Local nTamFil := Len(cFilAnt) //Thais Paiva - 24288418
Local _cQuery := ""
Local cShDtD  := ""
Local cShDtAt := ""
Local cShDemD := ""	
Local cShDemA := ""
Local nTp := 0 //Thais Paiva - 24288418
...
For nTp := 1 to Len(cShFiliais) Step nTamFil //8 Início Thais Paiva - 24288418
	cQryAux += "'"+SubStr(cShFiliais, nTp, nTamFil)+"'" //8)+"'"
	If (nTp + nTamFil) < Len(cShFiliais) //8) < Len(cShFiliais) Fim Thais Paiva - 24288418
		cQryAux += ","
	EndIf
Next nTp       
...
**************************
Local MvRetor	:= "" //Thais Paiva - 24288418
Local MvPar
Local MvParDef := ""
Local aItens   := {}
Local aArea    := GetArea()
Local nFor := 0 //Thais Paiva - 24288418
...
IF f_Opcoes  (@MvPar, "Opcoes", aItens     , MvParDef     ,12   , 49 , .F.        ,  8       ,   999    , .T.  )  // "Opções"
	...
	For nFor := 1 To Len( MvPar ) Step 2 //Início Thais Paiva - 24288418
...
cShFiliais:= mvRetor //MvParDef Fim Thais Paiva - 24288418
