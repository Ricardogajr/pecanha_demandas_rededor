		...
		*/
		...
		if (lNCriou	:= (len(aResul) == 0))
			...
			If !(_RetSql==0)
					...
					APMsgAlert(AllTrim(_RetSql),"Erro na criação da Tabela de Cliente X Mvto.Financeiros. Contactar Administrador do Sistema ")
			...
			Else 
				...
				If !(_RetSql==0)
						...
						APMsgAlert(AllTrim(_RetSql),"Erro na criação do indice da Tabela de Cliente X Mvto.Financeiros. Contactar Administrador do Sistema ")
				...
				Else
					if len(aResul	:= u_TabExist('TMPCLIENTE_MVTO_LOG')) == 0
						...
						If !(_RetSql==0)
								...
								APMsgAlert(AllTrim(_RetSql),"Erro na criação da Tabela de LOG Cliente X Mvto.Financeiros. Contactar Administrador do Sistema ")
						...
						Else 
							...
							If !(_RetSql==0)
									...
									APMsgAlert(AllTrim(_RetSql),"Erro na criação do indice da Tabela LOG de Cliente X Mvto.Financeiros. Contactar Administrador do Sistema ")
	...
	Local nRegAtu	:= 0
		...
		While !(cTabRet)->(Eof())
			...
			If !(_RetSql==0)
				...
				_RetSql := TcSQLError()
				APMsgAlert(AllTrim(_RetSql),"Erro na criação da Tabela. Contactar Administrador do Sistema ")
		...
		BEGIN TRANSACTION
			...
			if nAcao == 1
				...
				If !(_RetSql==0)
					...
					_RetSql := TcSQLError()
					APMsgAlert(AllTrim(_RetSql),"Erro na inclusão de linha de log. Contactar Administrador do Sistema ")
			...
			Elseif nAcao == 2
				...
				If !(_RetSql==0)
					...
					_RetSql := TcSQLError()
					APMsgAlert(AllTrim(_RetSql),"Erro na atualização da linha de log. Contactar Administrador do Sistema ")
