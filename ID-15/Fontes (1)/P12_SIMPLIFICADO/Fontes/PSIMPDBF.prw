...
User Function ImpDbf()
    Local bProc    := {|lAbort| StaticCall(IMPDBF,Exec,lAbort,aFiles) }
    Local aFiles   := {} 
    Private lAbort   := .F.
...
***********************************
   ...
   For x := 1 To Len(aFiles)
       ...
       DbUseArea(.T., "DBFCDXADS", FileNoExt(cFileName), cAliasx, .F., .F.)
       If (cAliasx)->(Eof())
          Loop
       Endif
       ...
       While (cAliasx)->(!Eof())
             ...
             TRYEXCEPTION
                ...
                For f := 1 To Len(aFields)
                    If SX3->(!DbSeek(PadR(aFields[f,1],nSeek)))
                       Loop
                    Endif
                    
                    xValue := (cAliasx)->(FieldGet((cAliasx)->(FieldPos(aFields[f,1]))))
                    If (SX3->X3_TIPO == "L") .And. ValType(xValue) != "L"
                       If Upper(xValue) == "T"
                          xValue := .T.
                       Else
                          xValue := .F.
...
********************************
    ...
    oDlg := TDialog():New(000, 000, 200, 400, "Selecione os arquivos a serem importados",,,,,,,,, .T.)
       ...
       oMark := MsSelect():New(cAlias,"OK","",aCpoBro,@lInverte,@cMark,{17,1,150,295},,,oDlg,,aCores)
