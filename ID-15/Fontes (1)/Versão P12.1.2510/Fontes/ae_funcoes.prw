...
User Function AEMontaEml(_cRotina, _nRecno,_cNomeDest,_cCodDest, _cStatus ,_cOpcao,oProcess,cPasta,cArqHtm)
...
User Function AEBuscaEml(_cCodDest)
...
User Function AEcalchora(_hTempo)
...
User Function AEDefDest(_nRecno, _cAprovacao,_nOpcao,_cTipo,_cOpcao)
Local _nDiasVia := 0
Local _cCodMun
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function AEConsisteData(periodo)
Local saldo := 0
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function AEMailConf(_nRecNo,_cstatus,_ctipo,_copcao,oProcess)
Local _nDiasVia := 0
...
oProcess  := U_AEMontaEml("H2",_nRecNo,_aDest[1],,_cstatus,_cOpcao,oProcess)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function AEMailFin(_nRecNo,_cTipo)
Local _nValorU
Local ni             
Local cErro			:= ""
Local lOK			:= .F.
...
For ni := 1 to Len(aEmailParam)
	aAdd(aEmailConfig,fGetPar(aEmailParam[ni][1],.F.,Nil))
...
nRet := oSrv:Init("", _MailS, aEmailConfig[2], aEmailConfig[3], 0, 25)
If nRet != 0
	lOk := .F.
Else
	lOk := .T.
EndIf
nRet := oSrv:SetSMTPTimeout(200)
...
If lOk 
	...
	If aEmailConfig[6]
		...
		nRet := oSrv:SmtpAuth(aEmailConfig[2], aEmailConfig[3])
		If nRet <> 0
			lAutentica := .F.
		Else
			lAutentica := .T.
	...
	If lAutentica          
        ...
        oMsg:cFrom    := aEmailConfig[4]
        oMsg:cTo      := _cMail
		oMsg:cCc      := _cMailCC
        oMsg:cSubject := _cSubject
        oMsg:cBody    := _cMsg
		...
		If !lOk                      
			...
			cErro := oSrv:GetErrorString(nRet)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function AEMlPrest(_cCodigo)
...
_aFunc	:= U_AEBuscaEML(LHP->LHP_FUNC)

//U_AEconsole("<--------FUNCAO PRESTACAO DE CONTAS ------------>")
...
Static Function fGetPar(cParam, lHelp, xPadrao)
Return SuperGetMV(cParam, lHelp, xPadrao)
/*
