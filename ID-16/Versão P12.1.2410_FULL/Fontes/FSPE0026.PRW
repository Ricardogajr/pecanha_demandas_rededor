...
/* Alterado por: Mauricio Siqueira
// Data: 12/09/24
// Motivo: Término da obrigatoriedade do PIS
*/
...
If _lRet 
	   ...
	   IF M->RA_TIPOADM $ "3A-3B-3C" .AND. M->RA_PERFGTS <> 2
	      _lRet := .F.  
         Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Para menor aprendiz e necessario o preenchimento do campo % Dep.FGTS = 2", 1, 0)
	   ENDIF    
   
	   IF !(M->RA_TIPOADM $ "3A-3B-3C") .AND. M->RA_PERFGTS = 2
	      _lRet := .F.  
         Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Para menor aprendiz e necessario o preenchimento do campo Tipo de Admissao", 1, 0)
...
//   IF lXPERINS = .F.  
//      _lRet := .F.  
//      Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor criar o campo RA_XPERINS", 1, 0)
//   ENDIF
//   IF lXTPINSA = .F.  
//      _lRet := .F.  
//      Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor criar o campo RA_XTPINSA", 1, 0)
//   ENDIF
//   IF lXINSALU = .F.  
//      _lRet := .F.  
//      Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor criar o campo RA_XINSALU", 1, 0)
//   ENDIF
//
//ENDIF
...
If valtype(M->RA_INSMIN)<>"N" 
    M->RA_INSMIN := 0
//   _lRet := .F.  
//   Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Verificar Hrs. Insulubridade Minima", 1, 0)
Endif      
If valtype(M->RA_INSMED)<>"N" 
   M->RA_INSMED := 0
//   _lRet := .F.  
//   Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Verificar Hrs. Insulubridade Media", 1, 0)
Endif      
If valtype(M->RA_INSMAX)<>"N" 
   M->RA_INSMAX := 0
//   _lRet := .F.  
//   Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Verificar Hrs. Insulubridade", 1, 0)
Endif      
If valtype(M->RA_PERICUL)<>"N" 
   M->RA_PERICUL := 0
//   _lRet := .F.  
//   Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Verificar Hrs. Periculosidade", 1, 0)
Endif      
...
IF _lRet
   ...
   IF (l_InsaC = .T.)  .AND. (l_InsaP = .T.) .AND. (l_PeriP = .T.)  .AND. (l_Insalu = .T.)
      _lRet := .F. 
      Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Voce preencheu os campos de Insabridade e Periculosidade, o funcionario so pode ter Isalubridade ou Periculosidade", 1, 0)
   ENDIF 
   
   // Nao pode estar preenchido as duas formas de Insalubridade
   IF (l_InsaC = .T.)  .AND. (l_InsaP = .T.) .AND. (l_Insalu = .T.) .AND. (l_PeriP = .F.) 
      _lRet := .F. 
      Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Voce preencheu os dois tipos de Insabridade, favor revisar os campos relativos a Insalubridade", 1, 0)
   ENDIF 
   ...
   IF l_ErroIns
      _lRet := .F.    
      Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Para calcular a Insalubridade especifica devemos preencher dois campos: Base de Insabridade e Percentual", 1, 0)
   ENDIF          
   
   IF l_TotInsa
      _lRet := .F.    
      Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - O funcionario so podera estar enquadrado em apenas uma Insalubridade (Minima, Media ou Maxima", 1, 0)
   ENDIF
...
If _lRet
   ...
   IF M->RA_NACIONA != "10" .And. M->RA_NACIONA != "20"                                      
      IF EMPTY(M->RA_RNE) .OR. EMPTY(M->RA_RNEORG) .OR. EMPTY(M->RA_RNEDEXP)
         Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor preencher Informações Sobre a RNE Pasta Estrangeiro", 1, 0)
         _lRet := .F.
      Endif
   Endif
   ...
   IF !M->RA_CATFUNC = "E"
      If Empty(M->RA_TPPREVI)
         Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor Preencher Tipo Regime Previdenciario", 1, 0)
         _lRet := .F.
      Endif 
	//================================ RETIRADO EM 23/10/2018 ==============
	//  If M->RA_SALARIO = 0
    //     Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor Preencher Salario", 1, 0)
    //     _lRet := .F.
	//  Endif
	...
	//======================================================================================
	 ...
	 For _nX3 := 1 to Len(_aCpoX3)
	  ...
	  Endif
    //======================================================================
/*	  If Empty(M->RA_PIS)   // Mauricio Siqueira - 12/09/24
         Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor Preencher Numero do PIS", 1, 0)
         _lRet := .F.
	  Endif
*/
   ENDIF
   ...
   IF M->RA_CLASEST != " " .OR. !EMPTY(M->RA_DATCHEG) 
      IF M->RA_CLASEST != "06"
         IF EMPTY(M->RA_DATCHEG)
            Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor preencher Data Chegada Pasta Estrangeiro", 1, 0)
            _lRet := .F.
         Endif
      ENDIF

      IF M->RA_CLASEST = "06" .AND. !EMPTY(M->RA_DATCHEG)
            Help(NIL, NIL, "GP010VALPE", Nil, "FSPE0026() - Favor Limpar Data Chegada Pasta Estrangeiro", 1, 0)
            _lRet := .F.
      Endif
   ENDIF
   /////////////////////////////////////////////////////////////////////
