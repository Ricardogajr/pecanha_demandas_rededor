	...
	If SC7->C7_CONAPRO = 'L'
		If SC7->C7_RESIDUO <> 'S'
			...
			if empty( alltrim(SA2->A2_EMAIL) ) .and. empty( alltrim(SC7->C7_XMAILFO) ) .and. empty( alltrim(SC7->C7_GRUPCOM) )
				...
				If SC7->C7_XORIG = '2'
					cEmailGrp := SC7->C7_XKTGRUP
				Else
					cEmailGrp := SC7->C7_GRUPCOM 
				EndIf

				dbSelectArea("SZJ")
				SZJ->( dbSetOrder(1) )     // ZJ_FILIAL + ZJ_GRCOM
				SZJ->( dbGoTop() )
				If SZJ->( dbseek( FWxFilial('SZJ') + cEmailGrp ) )
					cCopia := SZJ->ZJ_EMAIL
				EndIf
...
static function enviaemail( cSMTP , cConta , cPass , cContaDes , cAssunto , cArquivo , cMensagem , cMailContr , nRecSC7 )
	...
	if lConSMTP
		...
		If !oMsg:AttachFile( cNomeAnexo ) < 0
			oMsg:AddAtthTag( 'Content-Disposition: attachment; filename='+cNomeAnexo+'')
		EndIf
...
static function sndmailerr( cNumPC , nRecSC7 )
	...
	if lConSMTP
        ...
        oMsg:cFrom    := cConta
        oMsg:cTo      := alltrim( getmv( 'MV_XMAILFO' ))
        oMsg:cSubject := cAssunto
        oMsg:cBody    := cMensagem
