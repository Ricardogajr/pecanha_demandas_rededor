...
WSSTRUCT SolicitVag
	...
	WSDATA JusVaga		As String			OPTIONAL	//JusVaga
	WSDATA SolComp		As String			OPTIONAL	//SolComp
	WSDATA Locali		As String			OPTIONAL	//Locali
	WSDATA XOutros		As String			OPTIONAL	//XOutros
...
WsStruct VagDados
	...
	WSDATA JusVaga			As String			OPTIONAL	//Observação
	WSDATA SolComp			As String			OPTIONAL	//Observação
	WSDATA Locali			As String			OPTIONAL	//Observação
	WSDATA XOutros			As String			OPTIONAL	//Observação
...
WsService W0500308 Description "WebService Server para solicitação de vagas"
	...
	WSDATA JusVaga			As String optional	
	WSDATA SolComp			As String optional
	WSDATA Locali			As String optional	
	WSDATA XOutros			As String optional	
...
Static Function GrSolVgRh(oSolicVag,nOperacao,cFilSubd,cMatSubd,cVisPAB)
	...
	If nOperacao == 1
		...
		cLocali := IIf(oSolicVag:FilPosto $ "01310108|01310011",oSolicVag:Locali,"")
		aDados  := {{"QS_FILIAL"  , IIF(EMPTY(oSolicVag:FilPosto)   ,"",oSolicVag:FilPosto         )},;
                    ...
                    {"QS_XJUSTVA"   , IIF(EMPTY(oSolicVag:JusVaga) ,"",oSolicVag:JusVaga		   )},;
          			{"QS_XREQSOL"   , IIF(EMPTY(oSolicVag:SolComp)       ,"",oSolicVag:SolComp   )},;
                    {"QS_XLOCALI"   , cLocali },;
                    {"QS_XOUTROS"   , IIF(EMPTY(oSolicVag:XOutros)       ,"",oSolicVag:XOutros)};
                   }
		...
		If nOperacao == 1
			For nX:= 1 To Len(aDados)
				If !Empty(aDados[nX, 1])
					...
					If aDados[nX, 1] $ "TMP_XOBS|QS_XOUTROS"
...
WsMethod BscSolVags WsReceive CodSolic,Matricul,Visao,FilialS WsSend VDado WsService W0500308
	...
	If Len(aAux) > 0
		...
		*/
		...
		DbSelectArea("RH4")
		RH4->(DbSetOrder(1))
		If RH4->(DBSEEK(Alltrim(::VDado:FilVag) + ::CodSolic + "29"))
			::VDado:XOutros := RH4->RH4_XOBS
		Else
			::VDado:XOutros := ""
		EndIf
		...
		::VDado:JusVaga  := IIF((var := AScan(aAux, {|x, y|  Alltrim(x[1]) == "QS_XJUSTVA"})) = 0, '', aAux[var][2])
		::VDado:SolComp  := IIF((var := AScan(aAux, {|x, y|  Alltrim(x[1]) == "QS_XREQSOL"})) = 0, '', aAux[var][2])
		::VDado:Locali  := IIF((var := AScan(aAux, {|x, y|  Alltrim(x[1]) == "QS_XLOCALI"})) = 0, '', aAux[var][2])			
		//::VDado:XOutros  := IIF((var := AScan(aAux, {|x, y|  Alltrim(x[1]) == "QS_XOUTROS"})) = 0, '', aAux[var][2])			
...
WSMETHOD GetPostosD WSRECEIVE CompanyID, DepartmentID, PostID, Page, FilterField, FilterValue, RequestType, EmployeeFil, Registration, FilPotVis, PostoVis WSSEND PostDataD WSSERVICE W0500308
	...
	//cTabela := "%" + GetTableName("RCL", Self:CompanyId) + "%" Thais Paiva - 26084000
	...
	If tcSqlExec("SELECT RCL_FILIAL FROM RCL" + ::CompanyId + "0 WHERE RCL_FILIAL='*' ") >= 0
		...
		While !(cAuxAlias)->(Eof())	 .AND.;
			...
			BeginSql alias cAuxAlias1
				...
				AND RCX.%notDel%
				AND RCX.RCX_FILIAL = %exp:(cAuxAlias)->RCL_FILIAL% //%xfilial:RCX% Thais Paiva - 26084000
