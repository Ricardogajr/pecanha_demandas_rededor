...
Static Function WizCfgParam() 
    ...
    Private bPreview    := {|| PreviewLog(.F.,AllTrim((cAliasRes)->ZVJ_CODIGO),cDataIni,cDataIni,AllTrim((cAliasRes)->ARQLOG)) }
    Private lChkMark    := .F.
...
*************************
   ...
   While (cAliasRes)->(!Eof())
         ...
         AADD(aRet,"			<Full>"+cValToChar((cAliasRes)->FULL)+"</Full>")
...
********************************
	...
	aStruct := { {"CHECK"     , "L", 001                    , 00},;
		         ...
		         {"TEMPO"     , "C", 008                    , 00},;
		         {"ORDEM"     , "N", 010                    , 00},;
		         {"ARQLOG"    , "C", TamSx3("ZVJ_DIRIMP")[1], 00},;
		         {"ZVJ_CODIGO", "C", TamSx3("ZVJ_CODIGO")[1], 00},;
                 {"ZVJ_DESTIN", "C", TamSx3("ZVJ_DESTIN")[1], 00},;
                 {"ZVJ_TPIMP" , "C", TamSx3("ZVJ_TPIMP" )[1], 00},;
                 {"ZVJ_DESC"  , "C", TamSx3("ZVJ_DESC"  )[1], 00},;
		         {"ZVJ_DIRIMP", "C", TamSx3("ZVJ_DIRIMP")[1], 00}}
   
   If (Select(cAlias) == 0)
      //Cria a tabela temporária
      cFileRes := CriaTrab(aStruct,.T.)
      dbUseArea(.T.,RDDSetDefault(),cFileRes,cAlias,.F.,.F.)
      IndRegua(cAlias,cFileRes+"_01","ORDEM"     ,,," Aguarde, indexando os dados... ")
      IndRegua(cAlias,cFileRes+"_02","ZVJ_DESTIN",,," Aguarde, indexando os dados... ")
   ...
   While (cAliasTmp)->(!Eof())
         ...
         (cAlias)->FULL    := (nProcess > 0) .And. aProcess[nProcess,3]
         (cAlias)->VALID   := " "
         (cAlias)->STATUS  := " "
         (cAlias)->REFRESH := " "
...
*****************************
   ...
   BEGIN SEQUENCE
      ...
      For nX := 1 To Len(aRec)
          ...
          MsgRun( "Gerando o log... ("+cProcess+")" , "Aguarde..." , { || cFileLog := GeraRelat(cCodPrc,cDataIni,cHoraIni) } )
...
**********************************************************************
Static Function PreviewLog(lGerar,cCodProc,cDataRef,cHoraRef,cFileRpt)
**********************************************************************
   Local cLocalFile := "" 
   Local cFileName  := ""
   Local cProcesso  := ""
   Local bGeraRpt   := {|| cFileName := GeraRelat(cCodProc,cDataRef,cHoraRef) }
   Local lRet       := .T.
   Local lCopyOk    := .F.
   ...
   If lRet
      ...
      MsgRun( "Abrindo o arquivo de log...", "Aguarde...", { || lCopyOk := CpyS2T(cFileName,U_GetTmpPath(.T.)) } )
...
*****************************************************
Static Function GeraRelat(cCodProc,cDataRef,cHoraRef)
*****************************************************
   Local lRet       := .T.
   Local cTpImp     := AllTrim(ZVJ->ZVJ_TPIMP)
   Local cQryAna    := ""
...
***************************
   ...
   lRet := StaticCall(IMPTAB,CreateSeq,cSequence,cDestin)
   If !lRet
      return .F. 
   Endif
...
****************************
   ...
   While (cAliasRes)->(!Eof())
         ...
         IF (cAliasRes)->(RecLock(cAliasRes,.F.))
            (cAliasRes)->STATUS  := If(cTpImp == "5"," ",If(Empty(aFiles),"3"," "))
            (cAliasRes)->REFRESH := If(nRetSinc == 0," ",cValToChar(nRetSinc))
            (cAliasRes)->(MsUnLock())
         EndIf
         
         If lRet .And. ((cAliasRes)->STATUS == "3")
            lRet := .F.
         Endif
...
****************************
   ...
   Local aUnique    := StaticCall(DORCHARGE,GetUnique,cProcesso)
   Local cCmdInd    := ""
   ...
   If ! Empty(cCmdTab) 
      If (TcSQLExec(cCmdTab) != 0)
         APMsgAlert(AllTrim(TCSQLERROR()),'Não foi possível criar a tabela "'+cNomTab+'".')
         return .F.
      Endif
      ...
      If ! Empty(aUnique)
           ...
           If (TcSQLExec(cCmdInd) != 0)
              APMsgAlert(AllTrim(TCSQLERROR()),'Erro durante a criação do índice "'+cNomTab+'_BUSINESS".')
              return .F.
           Endif
      Endif
   Endif
   ...
   If ! Empty(cCmdLog) 
      If (TcSQLExec(cCmdLog) != 0)
         APMsgAlert(AllTrim(TCSQLERROR()),'Não foi possível criar a tabela "'+cNomLog+'".')
      Endif
   Endif

   If ! Empty(cCmdRes) 
      If (TcSQLExec(cCmdRes) != 0)
         APMsgAlert(AllTrim(TCSQLERROR()),'Não foi possível criar a tabela "'+cNomRes+'".')
      Else
         cCmdRes := "CREATE UNIQUE INDEX "+cNomRes+"_UNQ ON "+cNomRes+" (NumeroLote,Nome_Arquivo) "  
         If (TcSQLExec(cCmdRes) != 0)
            APMsgAlert(AllTrim(TCSQLERROR()),'Erro durante a criação do índice "'+cNomRes+'_UNQ".')
         Endif
      Endif
   Endif
...
***************************************
   ...
   While ZVK->(!Eof()) .And. ZVK->ZVK_FILIAL == ZVJ->ZVJ_FILIAL .And. ZVK->ZVK_CODEXT == ZVJ->ZVJ_CODIGO
        ...
        If SX3->(MsSeek(PadR(cCampo,nSeek)))
           cTam := cValToChar(SX3->X3_TAMANHO)
           Do Case 
              Case (SX3->X3_TIPO == "N")
