...
//aValores[7]//C7_SEGURO
	...
	AAdd(aCabec, {"C7_XUSRSP" , UsrFulName(cUserIdBk) /*cUserIdBk*/	 , Nil})//Lucas Miranda de Aguiar
	...
	For nI := 1 To oMdlDetail:Length()
		...
		AAdd(aItem, {"C7_XUSRSP" , UsrFulName(cUserIdBk) /*cUserIdBk*/						, Nil})//Lucas Miranda de Aguiar
					...
					if lRotCarga
						...
						Else
							...
							If lOpInc .Or. lOpUpd // 3 OU 4
								Reclock("SC7",.F.)
								SC7->C7_XUSRSP := UsrFulName(cUserIdBk) //cUserIdBk
...
User Function S0100414()
	...
	If MsgYesNo("Confirma alteração do solicitante [ " + AllTrim(UsrRetName(SC7->C7_XUSR)) + " ] para o novo solicitante [ " +;
		...
		SC7->C7_XUSRSP  := UsrFulName(cUserIdBk) //__cUserId
