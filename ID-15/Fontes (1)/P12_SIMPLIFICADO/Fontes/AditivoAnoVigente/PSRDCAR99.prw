...
Static function fValidaTxt(aArquivos)
	...
	For nA := 01 To Len(aArquivos)
		...
		*/
		...
		nPosItem := aScan(aCabec, {|x| AllTrim(Upper(x)) $ "C7_ITEM" })
		nPosTpSp := aScan(aCabec, {|x| AllTrim(Upper(x)) $ "C7_XTPSP" })


		aSort(aDados, , , {|x, y| x[nPosFil]+x[nPosTpSp]+x[nPosFor]+x[nPosLoj]+x[nPosSer]+x[nPosDoc]+x[nPosItem] < y[nPosFil]+y[nPosTpSp]+y[nPosFor]+y[nPosLoj]+y[nPosSer]+y[nPosDoc]+y[nPosItem]})
		cChaveAux := PadR(aDados[1][nPosFil], TamSx3("C7_FILIAL")[01]);
			...
			+PadR(aDados[nX][nPosSer], TamSx3("C7_XSERIE")[01])+PadR(aDados[nX][nPosDoc], TamSx3("C7_XDOC")[01]);
			+PadR(aDados[1][nPosTpSp], TamSx3("C7_XTPSP")[01])
		cFilBkp := cFilAnt
		oProcess:SetRegua2(len(aDados))
		For nX := 01 To len(aDados)
			if aDados[nX][nPosTpSp] $ "4|5"
				lXGpe := .T.
				cDocGpe := PadR(GetSx8Num("SD1", "D1_DOC"), TamSx3("D1_DOC")[1])
				cSerieGPE :=  "GPE"
				cChvDocS := cDocGpe + cSerieGPE
			else
				lXGpe := .F.
				cChvDocS := PadR(aDados[nX][nPosSer], TamSx3("C7_XSERIE")[01])+PadR(aDados[nX][nPosDoc], TamSx3("C7_XDOC")[01])
			endif
			...
			cChave :=  PadR(aDados[nX][nPosFil], TamSx3("C7_FILIAL")[01]);
				...
				+cChvDocS;
				+PadR(aDados[nX][nPosTpSp], TamSx3("C7_XTPSP")[01])
			...
			If cChave == cChaveAux
				...
				For nY := 01 To Len(aDados[nX])
					...
					if lExist
						if aCabec[nY] == "C7_XDOC" .And. lXGpe
							aAdd(aCabAux, {aCabec[nY], cDocGpe, Nil})
							Loop
						elseif aCabec[nY] == "C7_XSERIE" .And. lXGpe
							aAdd(aCabAux, {aCabec[nY], cSerieGPE, Nil})
							Loop
						endif
				...
				For nY := 01 To Len(aDados[nX])
					...
					if lExist
						if aCabec[nY] == "C7_XDOC" .And. lXGpe
							...
							Loop
						elseif aCabec[nY] == "C7_XSERIE" .And. lXGpe
							aAdd(aCabAux, {aCabec[nY], "GPE", Nil})
							Loop
						endif
						aAdd(aCabAux, {aCabec[nY], aDados[nX][nY], Nil})
					else
...
User Function RDCAR99A(cModel)
	Local cCmpCabec := ""
	oModel2 := FWLoadModel('PSF0100401')
