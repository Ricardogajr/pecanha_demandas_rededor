...
Static Function ImpIbsCbsIs( aNota, cClieFor, cLoja )
	...
	If DbSeek(xFilial("SD2")+aNota[2]+aNota[1]+cClieFor+cLoja)
		if !Empty(cDocumentItemId) .And. findClass('totvs.protheus.backoffice.fiscal.integration.taxinformation')
			...
			cXmlIBSCBS := oXmlRefTri:getXmlIBSCBS(AllTrim(SD2->D2_IDTRIB))
			cXmlIs := oXmlRefTri:getXmlIS(AllTrim(SD2->D2_IDTRIB))
			cXmlTotal := oXmlRefTri:getXmlTotalIbsCbsIs(AllTrim(SD2->D2_IDTRIB))

			If At("<imposto>", cXmlIBSCBS) > 0 .And. At("</imposto>", cXmlIBSCBS) > 0
				cXmlIBSCBS := StrTran( cXmlIBSCBS, "<imposto>", "" )
				cXmlIBSCBS := StrTran( cXmlIBSCBS, "</imposto>", "" )
				cXmlIBSCBS := "<impostoibscbs>" + cXmlIBSCBS + "</impostoibscbs>"
			EndIf

			If At("<imposto>", cXmlIs) > 0 .And. At("</imposto>", cXmlIs) > 0
				cXmlIs := StrTran( cXmlIs, "<imposto>", "" )
				cXmlIs := StrTran( cXmlIs, "</imposto>", "" )
