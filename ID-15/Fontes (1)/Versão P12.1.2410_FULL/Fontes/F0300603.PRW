	...
	*/
	...
	BeginSql Alias ctmpDep
		...
		WHERE SRB.RB_FILIAL BETWEEN %Exp:MV_PAR01% AND %Exp:MV_PAR02%
				...
				AND RHL.RHL_TPFORN = %Exp:ALLTRIM(STR(MV_PAR09))% //AND RHL.RHL_TPFORN = %Exp:MV_PAR09% Thais Paiva - 24287702
				AND SRB.%NotDel%
		ORDER BY %Order:SRB%
	EndSql
	...
	BeginSql Alias ctmpDep
		...
		WHERE	RHM.RHM_FILIAL BETWEEN %Exp:MV_PAR01% AND %Exp:MV_PAR02% AND
				...
				RHM.RHM_TPFORN = %Exp:ALLTRIM(STR(MV_PAR09))% AND  //RHM.RHM_TPFORN = %Exp:MV_PAR09% AND Thais Paiva - 24287702
				RHM.RHM_XSTAT = "2" AND
				RHM.%NotDel%
	EndSql
