...
Static Function fCHARGE_B(_cArquivo)
	...
	If  UPPER(Right(cPathTXT,4)) == ".TXT"
		lContinua := .T.
		aArqsTXT := StaticCall(IMPTAB,fDirectory,Left(cPathTXT,RAT("\",cPathTXT)),Substr(cPathTXT,RAT("\",cPathTXT)+1),.T.)
	ElseIf ExistDir(cPathTXT)
		lContinua := .T.
		aArqsTXT := StaticCall(IMPTAB,fDirectory,cPathTXT,Upper(Alltrim(cAliasTAB))+"*.TXT",.T.)
	Else
		...
		If Len(aArqsTXT) > 0
			...
			If lAutoExec .OR. MsgYesNo("Confirma a Migração de [ "+Alltrim(Str(Len(aArqsTXT)))+" ] Arquivo(s) TXT ???", "Tabela [ "+cNomTabTMP+" ]")
				...
				For nTXT := 1 To Len(aArqsTXT)
					...
					nTotLinha := StaticCall(IMPTAB,CountReg,0,aArqsTXT)
...
Static Function fCHARGE_Z2(nTXT)
	...
	SZ2->Z2_VALIDA  := "S"
	SZ2->Z2_MAXREC  := StaticCall(IMPTAB,GetMaxRecno,Upper(Alltrim(cAliasTAB)) )
