	...
	If oSolic:ValAprSol(cMat,cFil,cCodSol,cFilRh3)
		If oSolic:lVALAPRSOLRESULT
			...
			If cLocal == "008" //Férias // ticket n° 2829702 - 26/06/2018 - Paulo Dias  - variável atribúida
				...
				if !Empty(cAuthWS) //Início Thais Paiva - 24273898
					oInfPar:_HEADOUT :=  { "Authorization: BASIC "+ ENCODE64(rc4crypt( cAuthWS ,"AuthWS#ReceiptID", .F.,.T.)) }
				endif//Fim Thais Paiva - 24273898
...
Static Function SendEm(cAssunto,cBody,cEmail)
	...
	oWs := WSW0200301():new()
	if !Empty(cAuthWS) //Início Thais Paiva - 24273898
		oWs:_HEADOUT :=  { "Authorization: BASIC "+ ENCODE64(rc4crypt( cAuthWS ,"AuthWS#ReceiptID", .F.,.T.)) }
	endif//Fim Thais Paiva - 24273898
