...
**************************
Static Function fBefShow() //Before Show
**************************
      ...
      If (Type("oXmlPkg:_Package:_Params") == "O")
         ...
         For nX := 1 To Len(aNodes1)
             ...
             Aadd(aOldPar,{":"+aNodes1[nX,2]:RealName,aNodes1[nX,2]:_CAMPO:Text,aNodes1[nX,2]:_DSPAR:Text,cTipo,;
             	  aNodes1[nX,2]:_PICTURE:Text,aNodes1[nX,2]:_F3:Text,VAL(aNodes1[nX,2]:_TAMANHO:Text),VAL(aNodes1[nX,2]:_DECIMAL:Text),xValue})
         Next nX
      Endif
...
***********************
   ...
   While (cAliasRes)->(!Eof())
         ...
         */
         ...
         Aadd(aRet,StrTran("		<DECIMAL>{1}</DECIMAL>","{1}",CValToChar((cAliasRes)->DECIMAL)))
...
**********************************
   ...
   aParams   := StaticCall(CNCTALIB,GetParams,aCposSQL[1])
   ...
   While (cAliasRes)->(!Eof())
         Do Case
            ...
            Case ((cAliasRes)->TIPO == "N")
                 xDefault := Val("0" + If((cAliasRes)->DECIMAL > 0,"." + Replicate("0",(cAliasRes)->DECIMAL),""))
            Otherwise
...
*****************************************
   ...
   Local cPath     := U_GetDir(0) + cTabAlias+"\"
   Local nTotRec   := 0
   Local nRecno    := 0
   Local nFCount   := 0
   Local xValue    
...
***************************
   ...
   AADD(_stru,{"DESC" ,"C",040,000})
                           
   If (Select(cAlias) > 0)
     (cAlias)->(DbCloseArea())
   Endif
   
   cArqTmpBk := Criatrab(_stru,.T.)
   DBUSEAREA(.t.,,cArqTmpBk,cAlias)
   ...
   For nX := 1 To Len(aFields)
       ...
       (cAlias)->DESC   :=  GetX3Desc(aFields[nX,1])
   ...
   oMark       := MsSelect():New(cAlias,"OK","",aCpoBro,@lInverte,@cMark,{17,1,150,295},,,oWindow,,aCores)
...
***************************
   ...
   Local oMark     := Nil   
   Local cMark     := 'X1' //GetMark()
   Local lChkTodos := .F.
   Local oFontQtd  := TFont():New('Tahoma',,-018,,.T.)
   Local bRestore  := {|| EmpAll(.F.,oMark,cAlias,cMark), SetEmpsDef(cAlias,cMark) }
   ...
   cArqTmp := Criatrab(_stru,.T.)
   DBUSEAREA(.t.,,cArqTmp,cAlias)
   
   SM0->(DbGoTop())
   While SM0->(!Eof()) 
       RecLock(cAlias,.T.)
       (cAlias)->OK  	:=  Iif((AScan(aFiliais,{|x| x[1] == AllTrim(SM0->M0_CODIGO) .and. x[2] == AllTrim(SM0->M0_CODFIL) })>0),cMark,"")
...
****************************************
Static Function SetEmpsDef(cAlias,cMark)
****************************************
...
*********************************
   ...
   cAlias := PadR( MacExec(cMacro) ,Len((cAliasSx2)->X2_CHAVE))
         
   If (cAliasSx2)->(DbSeek( cAlias )) 
      cRet := AllTrim( (cAliasSx2)->X2_ARQUIVO )
   Endif
