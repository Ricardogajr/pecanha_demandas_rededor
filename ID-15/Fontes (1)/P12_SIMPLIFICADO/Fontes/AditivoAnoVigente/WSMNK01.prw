...
Static Function MNK01Pro(oSelf, lSchedule)
    ...
    If FWJSONDeserialize(cRetMNK, @oRetMNK)
        ...
        For nCont := 1 To nRegua
            ...
            If ValType(oRetMNK) == "O" .And. ValType(oRetMNK:companies[nCont]) == "O" 
                ...
                Else
                    lContinua := .F.
                    APMsgAlert("Não foi possível obter o CNPJ do profile. ")
                EndIf
            Else
                lContinua := .F.
                APMsgAlert("Não foi possível obter os dados do profile. ")
                Exit
            EndIf
                    ...
                    If "200 OK" $ cHeadGet2 .Or. "HTTP/1.1 200" $ cHeadGet2
                        If FWJSONDeserialize(cRetRec, @oRetRec)
                            If ValType(oRetRec) == "O"
                                ...
                                Else
                                    APMsgAlert(AllTrim(Upper(oRetRec:message)))
                                EndIf
                            Else
                                APMsgAlert("Não foi possível obter os dados da empresa. ")
                            EndIf
                        Else
                            APMsgAlert("Não foi possível obter os dados da empresa. ")
                        EndIf
	                    ...
	                    EndIf
                    Else
                        APMsgAlert("Erro na integração com a Receita Federal! ")
                    EndIf
    ...
    Else
        ...
        APMsgAlert("Erro na integração com a API Monkey! ")
