...
********************************
    Local bProc := {|lAbort| StaticCall(IMPTAB,ExecMain,cTab,cArq) }
    Private lAbort := .F.
...
***********************************
   ...
   Local cQuery := "SELECT 1 FROM all_sequences WHERE sequence_name = '{1}'"
   Local lRet   := .F.
...
*******************************************
	...
	For nX := 1 To Len(aAux)
		If ! SX3->( dbSeek(PadR( aAux[nX], nSize )) ) .OR. ( SX3->X3_CONTEXT == "V" ) 
		   ...
		   Loop
		Endif
		aAdd(aRet,{SX3->X3_ARQUIVO,SX3->X3_ORDEM, SX3->X3_CAMPO,SX3->X3_TIPO, SX3->X3_TAMANHO, SX3->X3_DECIMAL,SX3->X3_TITULO,;
							SX3->X3_VALID, SX3->X3_RELACAO,SX3->X3_CONTEXT, SX3->X3_OBRIGAT})
	Next 
	RestArea(aAreaSX3)
