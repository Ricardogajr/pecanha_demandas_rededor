...
For nX := 1 To Len(aTables)
    If SX3->(DbSeek(aTables[nX]))
       While SX3->(!Eof()) .And. SX3->X3_ARQUIVO == aTables[nX]
             Aadd(aModelo,{SX3->X3_CAMPO,SX3->(X3Titulo()),SX3->(X3DescriC()),.F.})
             SX3->(DbSkip(1))
       Enddo
    Endif
Next nX
...
******************************
   Local cFileName := U_GetTmpPath(.T.) + "AuditDic.csv"
