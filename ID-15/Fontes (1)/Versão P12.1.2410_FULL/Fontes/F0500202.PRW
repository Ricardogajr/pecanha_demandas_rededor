...
Criaãão do Menu de monitor das sOLICITAÇÕES
@Author     Fernando Carvalho
@Since      07/11/2016
@Version    P12.1.07
@Project    MAN0000007423039_EF_002
@Return		aRotina, opãão de menu
*/
Static Function MenuDef()
	...
	Aadd(aRotina , {"Visualizar"    , "VIEWDEF.F0500202"        , 0, 2})
	Aadd(aRotina , {"Cancelar Solicitação"    , "U_F0500225"        , 0, 6})	
...
Static Function CargaField(oModel)
	...
	Local aResult		:= {}
	ExecQuery()//Abre o arquivo temporãrio
	dbSelectArea(cAliasPA5)
	(cAliasPA5)->(dbGoTop())
...
Static Function Sair()	
	...
	oView:ButtonCancelAction()	//Necessãrio para encerrar a View 	
Return
...
Static Function ValidMod()	
	...
	If (cAliasPA5)->(! EOF())
		lRet 	:= .T.
	Else
		//Alert("Não hã indicadores para ser exibido.")
...
Validaãão para cancelamento da solicitação
		...
		If cNivel == "01" //RH
			...
			Else
				If cStatus == "1" .OR. cStatus == "4"
					...
					ElseIf cSolict == "006" //Movimentaãão Pessoal
	...
	If !Empty(aUser[1]) .AND. !Empty(aUser[2])
		...
		If SRA->(DbSeek(aUser[1]+aUser[2]))
			...
			If !lRh
				...
				// Erro percebido durantes testes para integraãão de alteraãão cadastral/contratual
				Else
