...
/*	cQUery += " AND NOT EXISTS (SELECT R_E_C_N_O_ FROM " + RetSqlName("P00")+ " P00 " + CRLF
	...
	While !(cAliasSE2)->(Eof())
		...
		if Empty((cAliasSE2)->P09_CODORI)
			cTexto := SuperGetMv("MV_XREF01",,"Recusa automática NF Material - Documento esta sem anexo.") 
			lRecusa := .T.
		endif

		if ndiasUteis <= cDiasRec
			cTexto += SuperGetMv("MV_XREF02",,"Recusa automática NF Material - Vencimento menor ou igual a "+cValToChar(cDiasRec)+" dias.") 			
			lRecusa := .T.			
		endif
