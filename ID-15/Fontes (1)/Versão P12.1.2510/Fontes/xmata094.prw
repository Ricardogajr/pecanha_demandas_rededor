			...
			If SCR->CR_TIPO == "MD"
				...
				If CNE->(DBSEEK(SCR->CR_FILIAL+AllTrim(SCR->CR_NUM)))	//SCR->CR_FILIAL	
					...
					If !(CNE->CNE_XMULTA  == 0.00 .And. CNE->CNE_XJUROS == 0.00)		
						...
						If (CNE->CNE_XMULTA >= nMulta.Or. CNE->CNE_XJUROS >= nJuros) .AND. SCR->CR_USER == getCodUsr() .AND. lContinua	
							//If lContinua
							cMsgMulJur := "Esta solicitação de pagamento possui valor de Multa ou Juros lançados. "  + Chr(13) + Chr(10)
...
//Criar a tabela temporária
	...
	//Cria a temporária
oTempTable := FWTemporaryTable():New(cArqTrb)
 

 
//Define as colunas usadasz
...
oTempTable:AddIndex("4", {"CR_FILIAL","CR_TIPO","CR_NUM" } )
