...
DbUseArea(.T., "TOPCONN", TcGenQry(, , cQry1), "cTMPx", .T., .T.)
While !cTMPx->(EOF())  
...
FOR NX := 1 TO LEN(ADADOS3)
    ...
    DbUseArea(.T., "TOPCONN", TcGenQry(, , cQry), "cTMPx", .T., .T.)
    While !cTMPx->(EOF())
      DBSELECTAREA("PZX")
      DBSETORDER(1)
...
DbUseArea(.T., "TOPCONN", TcGenQry(, , cQry1), cAlias2, .T., .T.)
