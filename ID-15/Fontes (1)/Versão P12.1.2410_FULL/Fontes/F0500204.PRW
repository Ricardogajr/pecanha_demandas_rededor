...
//  [ baseado nas respostas da tela de parï¿½metro   ]
// ------------------------------------------------------------------
...
user function relind1( PAR01 , PAR02 , PAR03 , PAR04 , PAR05 , PAR06 , PAR07 )
	...
	Local cPath      := AllTrim(GetTempPath()) //Thais Paiva 24909703
	local cArqXML    := criatrab( , .F. ) + '.XML' //alltrim( gettemppath() ) + criatrab( , .F. ) + '.xml' Thais Paiva 24909703
	...
	// ------------------------------------------------------------------
	//  oExcel:AddColumn( ''             , cNomTab , 'Nºmero FAP'          , 1 , 3 )
	// ------------------------------------------------------------------
	...
	If ARQSV1->(eof()) //Início Thais Paiva 24909703
		Alert("Relatório não retornou informações.")
		Return
	Else //Fim Thais Paiva 24909703
		do while ARQSV1->( !eof() )
			...
			// ------------------------------------------------------------------		
			//      [ verifica se há alï¿½ada do codigo da solicitação antes do periodo ]
			...
			// ------------------------------------------------------------------
			//      [ Fim verificação de peráodo anterior para solicitção ]
			// ------------------------------------------------------------------
			dbselectarea( 'ARQSV1' )
	...
	EndIf //Thais Paiva 24909703
	// -------------------------------
	//  [ gera planilha ]
	// -------------------------------
	oExcel:Activate()
	oExcel:GetXMLFile( cPath+cArqXML ) //( cArqXML ) Thais Paiva - 24909703
	oExcel:Deactivate() //Thais Paiva - 24909703
	// -------------------------------    
	...
	//SplitPath( cArqXML, @cDrive, @cDir, @cNome, @cExt ) Início Thais Paiva - 24909703
	
	//ShellExecute("Open", cNome+cExt, "", cDrive+cDir , 1 ) 
	ShellExecute("open", "excel.exe", cPath+cArqXML, "", 1) //Fim Thais Paiva - 24909703
...
user function relind2( PAR01 , PAR02 , PAR03 , PAR04 , PAR05 , PAR06 , PAR07 )
	...
	// -------------------------------
	local oExcel     := FWMsExcelex():New() // A classe FwMsExcelEx quando há mais de uma aba, a planilha corrompe.
	Local cPath      := AllTrim(GetTempPath()) //Thais Paiva - 24909703
	local cArqXML    := criatrab( , .F. ) + '.XML' //alltrim( gettemppath() ) + criatrab( , .F. ) + '.xml' Thais Paiva - 24909703
	...
	local dPA5xdat
	local cQry       := '' // gus
	//local lReg 		 := .F. //Início Thais Paiva 25508238
	local lReg1 := .F. 
	local lReg2 := .F.
	local lReg3 := .F. //Fim Thais Paiva 25508238
	Private  cDrive, cDir, cNome, cExt
	// -------------------------------
	...
	procregua( ARQSV1->( reccount() ))
	// -------------------------------
	If ARQSV1->(eof()) //Início Thais Paiva 25508238
		oExcel:AddRow( 'VSVSF' , cNomTab , {"Planilha com Solicitações do tipo",;
			"[Vagas Sem Codigo de Vagas e Sem FAP]","não retornou informações.", ;
			"","","","","","","","","","","","","","","","",""} )
		lReg1 := .T. //Thais Paiva - 25508238
	Else //Fim Thais Paiva 25508238
		do while ARQSV1->( ! eof() )
			//lReg := .T. Thais Paiva 25508238
			...
			do while ARQSV1->PA5_XNRSOL == cPA5Xnrsol
				...
				cTipCod :=       ARQSV1->PA5_XCODEV
				cDesTip :=       ARQSV1->PA5_XDESEV
				cFunc   := StrTran(ARQSV1->PA5_FUNCAO,"&","E")
				cMat    :=       ARQSV1->PA5_XMAT
				cNome   :=       ARQSV1->PA5_XNOME
				cSet    := StrTran(ARQSV1->PA5_XSETOR,"&","E")
				cCarg   := StrTran(ARQSV1->PA5_XCARGO,"&","E")
				cMatAp  :=       ARQSV1->PA5_XMATAP
				cAprov  :=       ARQSV1->PA5_XAPROV
				cSetApr := StrTran(ARQSV1->PA5_XSEAPV,"&","E")
				cUniApr :=       ARQSV1->PA5_XUNAPR
				cCarApr :=       ARQSV1->PA5_XCARAP
				cStat   :=       ARQSV1->PA5_XSTAT
				dData   := stod( ARQSV1->PA5_XDATA ) // ARQSV1->(STOD(PA5_XDATA))
		...
		enddo
	endif //Thais Paiva - 25508238
	ARQSV1->( dbclosearea() )
	
	If !lReg1 //Imício Thais Paiva - 25508238
		oExcel:AddRow( 'VSVSF' , cNomTab , {"Planilha com Solicitações do tipo",;
				"[Vagas Sem Codigo de Vagas e Sem FAP]","não retornou informações.", ;
				"","","","","","","","","","","","","","","","",""} )
	endif //Fim Thais Paiva - 25508238
	// ------------------------------------------------------------------
	...
	If ARQSV1->(eof()) //Início Thais Paiva 25508238
		oExcel:AddRow( 'VCVSF' , cNomTab , { "Planilha com Solicitações do tipo",;
					"[Vagas Com Codigo de Vagas e Sem FAP]","não retornou informações.",;
					"","","","","","","","","","","","","","","","",""} )
		lReg2 := .T. //Thais Paiva 25508238
	Else //Fim Thais Paiva 25508238
		do while ARQSV1->( ! eof() )
			//lReg := .T. Thais Paiva 25508238
			// -------------------------------
			...
			if SQS->( dbseek( cPA5Xnrsol ))
				...
				dbselectarea( 'PA2' ) // FORMULï¿½RIO DE APROVAÇÃO PROFIS
				...
				if !PA2->( dbseek( cQSFilVg + cQSVaga ))
					...
					// ------------------------------------------------------------------
					//              [ essa verificação ï¿½ a mais importante. ]
					//              [ sï¿½ segue se nao existir na FAP        ]
					// ------------------------------------------------------------------
					dbselectarea( 'ARQSV1' )
	...
	ARQSV1->( dbclosearea() ) //Thais Paiva - 25508238

	If !lReg2 //Início Thais Paiva - 25508238
		oExcel:AddRow( 'VCVSF' , cNomTab , { "Planilha com Solicitações do tipo",;
					"[Vagas Com Codigo de Vagas e Sem FAP]","não retornou informações.",;
					"","","","","","","","","","","","","","","","",""} )
	endif //Fim Thais Paiva - 25508238
	...
	oExcel:AddColumn( 'VCVCF' , cNomTab , 'Nºmero FAP'          , 1 , 1 )
	...
	If ARQSV1->(eof()) 
		oExcel:AddRow( 'VCVCF' , cNomTab, { "Planilha com Solicitações do tipo",;
						"[Vagas Com Codigo de Vagas e Com FAP]","não retornou informações.",;
						"","","","","","","","","","","","","","","","","",""} )
		lReg3 := .T. //Thais Paiva 25508238
	Else //Fim Thais Paiva 25508238
		do while ARQSV1->( ! eof() )
			//lReg := .T. Thais Paiva 25508238
			...
			if SQS->( dbseek( cPA5Xnrsol )) // verifica se existe na SQS(vagas) , por redundancia
				...
				// ------------------------------------------------------------------
				//          [ verifica na PA2(FAP). sï¿½ segue se existir na PA2. ]
				...
				if PA2->( dbseek( cQSFilVg + cQSVaga ))
					...
					// ------------------------------------------------------------------
					//              [ Fim verificação de peráodo anterior para solicitação ]
					// ------------------------------------------------------------------
	...
	If !lReg3 //Início Thais Paiva - 25508238
		oExcel:AddRow( 'VCVCF' , cNomTab, { "Planilha com Solicitações do tipo",;
						"[Vagas Com Codigo de Vagas e Com FAP]","não retornou informações.",;
						"","","","","","","","","","","","","","","","","",""} )
	EndIf 
	If !lReg1 .AND. !lReg2 .AND. !lReg3 //Fim Thais Paiva - 25508238
		//oExcel:AddworkSheet(' ') 
		//oExcel:AddTable( ' ' , ' ' )
		//oExcel:AddColumn( ' ' , ' ' , ' ' , 1 , 1 )
		//oExcel:AddRow( ' ' , ' ' , { ' ' } )
		Alert("Relatório não retornou informações.")
		Return
	EndIf 
	...
	//  [ gera planilha ]
	// -------------------------------
	  oExcel:Activate() //Reativado Trecho Thais Paiva - 24909703
	  oExcel:GetXMLFile( cPath+cArqXML ) //( cArqXML ) Reativado Trecho Thais Paiva - 24909703
	  oExcel:Deactivate() //Thais Paiva - 24909703
	//// -------------------------------
	//    oExcelApp := MsExcel():New()
	//    oExcelApp:WorkBooks:Open( cArqXML )
	...
	//SplitPath( cArqXML, @cDrive, @cDir, @cNome, @cExt ) Inicio  Thais Paiva - 24909703
	//ShellExecute("Open", cNome+cExt, "", cDrive+cDir , 1 )
	ShellExecute("open","excel.exe", cPath + cArqXML, "", 1) //Fim Thais Paiva - 24909703
...
user function relind4( PAR01 , PAR02 , PAR03 , PAR04 , PAR05 , PAR06 , PAR07 )
	...
	// ------------------------------------------------------------------
	//  oExcel:AddColumn( 'FAP' , cNomTab , 'Nºmero FAP'          , 1 , 3 )
	// ------------------------------------------------------------------
...
//	local oReport
//	local oSection
//	local lLandscape := .T. //-Aponta a orientação de pï¿½gina do relatório como paisagem
//	
//	if Pergunte("FSW0500204",.t.)
//		
