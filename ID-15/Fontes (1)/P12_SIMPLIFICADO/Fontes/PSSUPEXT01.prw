...
Local lRet:= .T. , nCampos,  aArea := GetArea() 
Local aValidSx3 := {}             
...
Local aArea := GetArea()
Local aAreaSave := (cpAlias)->( GetArea() )
...
Static function AtuVld(cpField)                    
	...
	Local cVldAll := ''
		...
		if SX3->(dbSeek(cpField))
			
			cVldPro	:= Alltrim(SX3->X3_VALID)
			cVldUsr	:= Alltrim(SX3->X3_VLDUSER)
			...
			if Empty( ZVK->ZVK_VLDPRO )
			   RecLock("ZVK",.F.)                                     
				...
				//ZVK->ZVK_TAMDES		:= Alltrim(Str(SX3->X3_TAMANHO))
				//ZVK->ZVK_DECDES		:= SX3->X3_DECIMAL
				//ZVK->ZVK_OBRDES		:= SX3->X3_OBRIGAT
				//ZVK->ZVK_IMPDES		:= SX3->X3_RESERV//ANALISAR
				//ZVK->ZVK_PREDES		:= SX3->X3_F3
				//ZVK->ZVK_VIRTUA		:= IIF(SX3->X3_CONTEXT=='V','S','N')
