...
User Function F0100401()
	...
	IF !Empty(cSoliPag) .And. __CUSERID $ cSoliPag
		...
		If U_fVldRmed()
			//cFiltroRP := "(C7_XREPMED == '2' .Or. C7_XREPMED == '3' .Or. C7_XREPMED == '1' .Or. C7_XREPMED == ' ')"
			oBrw:AddFilter("Usr","C7_XUSR $ '"+cUserP32+"'",.T.)
		Else
			oBrw:AddFilter("Usr","C7_XUSR $ '"+cUserP32+"'",.T.)
			oBrw:AddFilter("Rp1","C7_XREPMED == '1' .Or. C7_XREPMED == ' ' ",.T.)
		EndIf
		//oBrW:AddFilter("UsrR","C7_XUSR $ '"+fGetUsrR()+"' .And. (C7_XREPMED == '2' .Or. C7_XREPMED == '3')",.T.)
		/*/If U_fVldRmed()
		oBrw:SetFilterDefault("(C7_XSOLPAG == '1' .And. C7_XUSR $ '"+cUserP32+"' .And. C7_FILIAL $ '" +cFils+"') .AND. (C7_XREPMED == '2' .Or. C7_XREPMED == '3')" )
	Else
		oBrw:SetFilterDefault("C7_XSOLPAG == '1' .And. C7_XUSR $ '"+cUserP32+"' .And. C7_FILIAL $ '" +cFils+"' .And. (C7_XREPMED != '2' .Or. C7_XREPMED != '3')" )
		EndIf/*/
		//oBrw:SetFilterDefault("C7_XSOLPAG == '1' .And. C7_XUSR $ '"+cUserP32+"'" )
		/*/aUsersP32 := strtokarr(cUserP32,"|")
		cFiltro := "C7_XSOLPAG == '1' .And. ("
		For nX := 01 To Len(aUsersP32)
			If nX == Len(aUsersP32)
				cFiltro := cFiltro + "C7_XUSR == '" +aUsersP32[nX] + "')"
			Else
				cFiltro := cFiltro + "C7_XUSR == '" +aUsersP32[nX] + "' .Or. "
			EndIf
		Next nX/*/
		...
		Else
			...
			//	oBrw:AddFilter("Rp1","C7_XREPMED == '2' .Or. C7_XREPMED == '3' .Or. C7_XREPMED == '1' .Or. C7_XREPMED == ' '",.T.)
			//cFiltroRP := "(C7_XREPMED == '2' .Or. C7_XREPMED == '3')"
			//oBrw:SetFilterDefault("((C7_XSOLPAG == '1' .And. C7_XUSR == '"+__CUSERID+"') .Or. C7_XSOLPAG == '1' .And. "+cFiltroRP+")")
			//Else
			oBrw:AddFilter("SOLPAG","C7_XSOLPAG == '1'",.T.)
			oBrw:AddFilter("CUSR","C7_XUSR == '"+__CUSERID+"'",.T.)
			//oBrw:AddFilter("Rp1","C7_XREPMED == '1' .Or. C7_XREPMED == ' ' ",.T.)
			//oBrw:SetFilterDefault("C7_XSOLPAG == '1' .And. (C7_XREPMED == '1' .Or. C7_XREPMED == ' ') .And. C7_XUSR == '"+__CUSERID+"'")
			//EndIf
		Endif
	...
	*/
		...
		oBrw:AddLegend("C7_QUJE == 0 .And. C7_QTDACLA == 0 .AND. C7_CONAPRO = 'L' .AND. !U_FRecSF1() .And. !U_FRecMat()"	, "ENABLE"	 , "Pendente"                 ) // Aqui está o problema, está é legenda com problema

		oBrw:AddLegend("C7_CONAPRO == 'R' .And. C7_QUJE < C7_QUANT", "BR_PINK" 					, "Recusa Por Alcada"             )
		oBrw:AddLegend("(C7_QTDACLA > 0 .AND. C7_CONAPRO = 'L' .AND. U_FRecSF1()) .Or. U_FRecMat()","BR_CANCEL"	 	 			, "Recusa Doc. Entrada"             )
...
Static Function MenuDef()
	...
	ADD OPTION aRotina TITLE "Pesquisar"  ACTION "VIEWDEF.PSF0100401" OPERATION 1  ACCESS 0 // Pesquisar
	ADD OPTION aRotina TITLE "Visualizar" ACTION "VIEWDEF.PSF0100401" OPERATION 2  ACCESS 0 // Visualizar
	ADD OPTION aRotina TITLE "Incluir"	  ACTION "VIEWDEF.PSF0100401" OPERATION 3  ACCESS 0 // Incluir
	ADD OPTION aRotina TITLE "Alterar"	  ACTION "VIEWDEF.PSF0100401" OPERATION 4  ACCESS 0 // Alterar
	ADD OPTION aRotina TITLE "Excluir"	  ACTION "VIEWDEF.PSF0100401" OPERATION 5  ACCESS 0 // Excluir
...
Static Function ModelDef()
	...
	Local aTrigCond2 :=	FwStruTrigger("C7_COND" , "C7_XDTVEN",  "U_F0100411(M->C7_XDTEMI, M->C7_COND)", .T., "SE4",  01, "xFilial('SE4') + M->C7_COND"   , )
	...
	Local aTrigTipo2 := FwStruTrigger("C7_XTIPO"  , "C7_XTIPOD", "StaticCall(PSF0100401, FRETENC)"	   , .T., "P02",  01, "xFilial('P02') + M->C7_XTIPO"  , )
	...
	Local aTrigEspe :=	FwStruTrigger("C7_XTPSP", "C7_XESPECI" , "StaticCall(PSF0100401, GetEspec)"    , .F.,      ,    ,                                 , )
...
User Function F0100410(oModel)
	...
	Local aAreas     := {SAJ->(GetArea()),P02->(GetArea()),SY1->(GetArea()), GetArea()}
	Local nOper		 := oModel:GetOperation()
	Local cComprador := ""
	Local lRet       := .T.
	Local lSolic     := .T.
...
Static Function ViewDef()
	...
	Local oModel	:= FWLoadModel("PSF0100401")
	...
	oView:AddUserButton('Consulta DT.Fixa','Consulta DT.Fixa - Visualizar', {||  U_PSDTFIXSP(SC7->C7_COND, SC7->C7_TOTAL, 1) } )
...
Static Function SaveModel(oModel)
	...
	If lMJ .And. !FwIsInCallStack("U_RDCAR98")
					...
					if FwIsInCallStack("U_RDCAR02")
						...
						Else
							...
							If lOpInc .Or. lOpUpd // 3 OU 4
								...
								If AllTrim(SC7->C7_XTPSP) $ "2|3" .And. AllTrim(SC7->C7_XDTEXCE) == "1"//Verifica se é SP de material
...
Static Function ExcluiNota(nRecno, cTipoSp)
	...
	if SE2->(DbSeek(cChavE2))
		//if SC7->C7_XTPSP $ "2|3" .And. SC7->C7_XRATSP == "S" .And. !Empty(SE2->E2_ARQRAT)
		//SetFunName("FINA050")
		//RateioFina(cChavE2, SE2->E2_ARQRAT, 5, '512')
		//SetFunName("F0100401")
		//endif
		If cTipoSP == "4"
			RecLock("SE2", .F.)
...
User Function TrataXSer()
	...
	if FWIsInCallStack("U_RDCAR02") .And. FwIsInCallStack("U_RDCAR98")
		Return .T.
	endif
...
Static Function gerNotaFis(nRecno)
	...
	ExcluiNota(nRecno, SC7->C7_XTPSP)
...
//	MATA103(aCabec,aItens,3,,,,,/*aColsCC*/,,,/*aCodRet*/) //ExpN1 - Op?o desejada: 3-Inclus?; 4-Altera?o ; 5-Exclus?return
	...
	if lMsErroAuto
		...
		if SC7->C7_XTPSP $ "2|3" .And. SC7->C7_XRATSP == "S"
			RateioFina(cChavE2)
		endif
		if SC7->C7_XRATSP != "S"
...
User Function ContabSP(cLp, cChave)
	...
	If lLanctoOn
		...
		nValLancto := DetProva(nHdlPrv, cLp, "F0100401", cLoteEst, @nLinha)
...
Static function GetDesFin()
	...
	if oModel:GetOperation() == 4
		if SC7->C7_XTPSP $ "2|3" //Tipo material
			nDesc := SC7->C7_VLDESC
		endif
	endif
