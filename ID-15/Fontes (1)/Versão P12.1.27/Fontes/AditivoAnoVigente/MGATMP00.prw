...
Static Function MntCpTMP(cCposTMP)
    ...
    AEval(aCposAtu,{|c| c[4] := If(Empty(c[4]) .And. SX3->(DbSeek(PadR(c[1],nSeek))),cValToChar(SX3->X3_TAMANHO),c[4])})  
...
Static Function CriaTMP(cCposTMP,lNMenu)
	...
	Local _RetExec
	Local aUnique   := StaticCall(DORCHARGE,GetUnique,cTabZVJ)
	

	If MsFile(cNmTmpTb) .And. ( TcSQLExec("DROP TABLE "+cNmTmpTb) != 0 )
	   APMsgAlert(AllTrim(TCSQLERROR()),"Não foi possível recriar a tabela, a rotina não continuará sendo executada!")
	   return .F.
	Endif
	...
	if !MsFile(cNmTmpTb+'_LOG')
	   ...
	   If (TcSQLExec(cCriaTMP) != 0)
          APMsgAlert(AllTrim(TCSQLERROR()),". Não foi possível criar a tabela de LOG, a rotina não continuará sendo executada!")
          return .F.
	   Endif
	Endif  
	...
	if !MsFile(cNmTmpTb+'_RESUMO')
		...
		If !(_RetExec==0) 
			...
			if !lNMenu
				...
				APMsgAlert(AllTrim(_RetExec),". Não foi possível criar a tabela, a rotina não continuará sendo executada!")
		...
		Else
			...
			If !(_RetExec==0) 
				...
				if !lNMenu
					...
					APMsgAlert(AllTrim(_RetExec),"Não foi possível criar o indice, a rotina não continuará sendo executada e a tabela criada será cancelada!")
		...
		/*
			...
			If !(_RetExec==0) 
				...
				if !lNMenu
					...
					APMsgAlert(AllTrim(_RetExec),"Não foi possível criar o índice, a rotina não continuará sendo executada!")
