...
//   ValidPerg(cPerg)
  ...
  cPath := If(Right(cPath,1) == "\",cPath,cPath+"\") //Thais Paiva - 24368568
...
********************************     
Local nTamFil := Len(cFilAnt) //Thais Paiva - 24368568
Local cRaca     := ""
Local _cQuery   := ""
Local lDemitidos:= .F.
Local lTransfer := .F.
...
Local nTp := 0 //Thais Paiva - 24368568
Local _nRa := 0 //Thais Paiva - 24368568
 
nArqTxt := MsFCreate(cxLocArq) 
  
If nArqTxt <  0 .OR. !File(cxLocArq) //nArqTxt == -1 Thais Paiva - 24368568 
...
For nTp := 1 to Len(cShFiliais) Step nTamFil //8 Início Thais Paiva - 24368568
	cQryAux += "'"+SubStr(cShFiliais, nTp,  nTamFil)+"'" //8)+"'"
	If (nTp + nTamFil) < Len(cShFiliais) //8) <= Len(cShFiliais)  Fim Thais Paiva - 24368568
		cQryAux += ","
	EndIf
Next nTp 
...
//      cDetArq += PadR(TSRA->RA_CC,11,"")												            		+";"
    ...
    If Len(_aDadosCom) > 0
    	FClose(nArqTxt)
		ShellExecute( "Open" , cTime ,"", cPath , 3 ) //ShellExecute( "Open" , cxLocArq ,"", "" , 3 )  Thais Paiva 24368568    
    Endif
...
**************************
Local nFor := 0 //Thais Paiva - 24368568
Local MvPar
Local MvParDef := ""
Local aItens   := {}
Local aArea    := GetArea() 
Local MvRetor	:= "" //Thais Paiva - 24368568
Local lRet      := .F.
...
IF f_Opcoes  (@MvPar, "Opcoes", aItens     , MvParDef     ,12   , 49 , .F.        ,  8       ,   999    , .T.  )  // "Opções"
	For nFor := 1 To Len( MvPar ) Step 2 //Início Thais Paiva - 24368568
		If ( SubStr( MvPar , nFor , 2 ) # "**" )
			mvRetor += SubStr( MvPar , nFor , 2 ) 
		EndIf
	Next nFor
	
	If Empty(Alltrim(mvRetor))
		mvRetor := Space(len(MvPar)) 
	EndIf 
	
	&MvRet := mvRetor //MvPar                                      // Devolve Resultado
EndIF

cShFiliais:= mvRetor //MvParDef Fim Thais Paiva - 24368568
