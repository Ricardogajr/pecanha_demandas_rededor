...
***************************
   ...
   // := "X2_CHAVE"
    ...
    cQryX2 += " AND X2_CHAVE   = '"+SX2->(FieldGet(FieldPos('X2_CHAVE')))+"'"
    cQryX2 += " AND X2_ARQUIVO = '"+SX2->(FieldGet(FieldPos('X2_ARQUIVO')))+"'"
...
********************************
   ...
   */
   ...
   For nF := 1 To Len(aRet)
      ...
      aRetorno := startjob("u_fExecMac",getenvserver(), .T., cEmpAux, cFilAnt, cToken, aMacros, cQuery)
   ...
   */
   ...
   If Empty(aRet)
      aRet := {{cEmpAux,cFilAnt,RetSqlName(cTabAlias),cQuery}}
   Endif
   
Return aRet

User Function fExecMac(cEmp, cFil, cToken, aMacros, cQuery)
   Local cQry := ""
   Local cTable := ""
   Local nX
