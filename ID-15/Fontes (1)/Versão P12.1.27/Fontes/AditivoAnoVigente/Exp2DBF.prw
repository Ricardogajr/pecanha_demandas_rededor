...
Aadd(aFiles,'P16010')
   ...
   For x := 1 To Len(aFiles)
       ...
       For f := 1 To (cAlias)->(FCount())
           if SX3->(dbSeek(PadR((cAlias)->(FieldName(f)),nSeek))) .And. SX3->X3_TIPO $ "D|N"
              do Case
                 case (SX3->X3_TIPO == "D")
                      TCSetField(cAlias,(cAlias)->(FieldName(f)),"D")
                 case (SX3->X3_TIPO == "N")
                      TCSetField(cAlias,(cAlias)->(FieldName(f)),"N", 15 /*SX3->X3_TAMANHO*/,SX3->X3_DECIMAL)
              endCase
           endif
       Next f
