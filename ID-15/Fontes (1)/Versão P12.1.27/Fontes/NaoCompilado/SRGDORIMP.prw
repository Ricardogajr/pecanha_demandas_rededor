...
±±?Programa  ?SRGDORIMP                              ?Data ?19/07/2015 º±?
...
±±?Descricao ?Rotina para a importação das Rescisões do template CSV     º±?
...
Define MsDialog oDlg Title "Importação das Rescisões" From 08,15 To 018,080 Of GetWndDefault()
...
If lOk
	oProcess:=MsNewProcess():New( { |lEnd| lConv:= u_ImpSrgCSV(cArquivo, @lEnd)}, "Importação das Rescisões", "Processando arquivo das Rescisões", .T. )
	oProcess:Activate()
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±?Programa  ?IMPSRgCSV                              ?Data ?19/07/2015 º±?
±±ÌÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±?Autor     ?Microsiga                                                  º±?
±±ÌÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±?Descricao ?Rotina para a importação das Rescisões do template CSV     º±?
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
User Function ImpSrgCSV(cArq, lEnd)
...
While nLinTit > 0 .AND. !Ft_FEof()
   ...
   If LEN(cLinha) == 1023
	...
	If nLinTit == 1
		...
		If u_VERTEMPL(aCampos[1], "SRG", "RG")
			Return .F.
		EndIf
		
		nPosFil := aScan(aCampos,{ |x| Upper(AllTrim(x)) == "RG_FILIAL" })
		nPosMat := aScan(aCampos,{ |x| Upper(AllTrim(x)) == "RG_MAT" })
		nPosDem := aScan(aCampos,{ |x| Upper(AllTrim(x)) == "RG_DATADEM" })
	EndIf
	AADD( aErros, cLinha )
	cLinha := ""
	Ft_FSkip()
...
Do While !FT_FEOF()
	...
	If u_VldChave("SRG", aCampos, aDados)
			...
			*/
			...
			If lObrigat
				If EMPTY(aDados[W]) .OR. ALLTRIM(aDados[W]) == ""
					...
					AADD(aLog,{"RG_MAT"  ,aDados[nPosMat] ,Nil})
			...
			If lObrigat
				If aCampos[W] == "RG_DATAHOM"	// fChkDtHom(M->RG_DATADEM , M->RG_DATAHOM)
					// FCHKDTHOM() - FONTE - GPEM040.PRX
					// Checar a Data de Homologacao do Funcionario
					If VALTYPE(aDados[nPosDem]) <> "D"
						If AT("/", aDados[nPosDem]) > 0
							dDtDem := IIf(EMPTY(aDados[nPosDem]),CTOD("  /  /    "),CTOD(aDados[nPosDem]))
						Else
							dDtDem := IIf(EMPTY(aDados[nPosDem]),CTOD("  /  /    "),STOD(aDados[nPosDem]))
						EndIf
					Else
						dDtDem := aDados[nPosDem]
					EndIf
				    If aDados[W] < dDtDem
						cErro := cLinha+";DATA DE HOMOLOGACAO MENOR QUE A DATA DE DEMISSAO - "+cCampo+" - "+DTOS(aDados[W])
						AADD( aErros, cErro )
						
						AADD(aLog,{"RG_MAT"  ,aDados[nPosMat] ,Nil})
						AADD(aLog,{"ERRO" 	 ,"DATA DE HOMOLOGACAO MENOR QUE A DATA DE DEMISSAO - "+cCampo+" - "+DTOS(aDados[W]) ,Nil})
				...
				EndIf
				/*
				If aCampos[W] == "RG_DTGERAR"	// ValidGeracao()
					// ValidGeracao() - FONTE - GPEM040.PRX
					// Realiza verificação na data de Geração geradno alerta na tela. 
				EndIf
				*/
				If aCampos[W] == "RG_NORMAL" .OR. aCampos[W] == "RG_DESCANS"	// M->RG_NORMAL   > 0.00
					If aDados[W] <= 0
						cErro := cLinha+";VALOR MENOR OU IGUAL A ZERO"+cCampo+" - "+STR(aDados[W])
						AADD( aErros, cErro )
						
						AADD(aLog,{"RG_MAT"  ,aDados[nPosMat] ,Nil})
						AADD(aLog,{"ERRO" 	 ,"VALOR MENOR OU IGUAL A ZERO"+cCampo+" - "+STR(aDados[W]) ,Nil})
						
						lGrava := .F.
						LOOP
					EndIf
				EndIf
				If aCampos[W] == "RG_JTCUMPR"	// Pertence("12")
					If aDados[W] <> "1" .AND. aDados[W] <> "2"
						cErro := cLinha+";O CONTEUDO DO CAMPO "+cCampo+" NAO PERTENCE AO RANGE 'AMI ' - "+aDados[W]
						AADD( aErros, cErro )
						
						AADD(aLog,{"RG_MAT"  ,aDados[nPosMat] ,Nil})
						AADD(aLog,{"ERRO" 	 ,"O CONTEUDO DO CAMPO "+cCampo+" NAO PERTENCE AO RANGE 'AMI ' - "+aDados[W] ,Nil})
    ...
    If lGrava
		u_GRVDADOS("SRG", aCampos, aDados, nPosFil)
	Else
		lGrava := .T.
	EndIf
